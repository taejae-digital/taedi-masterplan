  ...
  return (
    <div style={{ pageBreakBefore: "always", pageBreakAfter: "always", minHeight: "280mm" }}>
      ...
      <div style={{ padding: "0 32px 14px" }}>
        <p style={ps}>
          산업화의 핵심은 분업이었다. 애덤 스미스가 핀 공장에서 읽어낸 이후 200년간, 생산성의 원천은 작업을 더 잘게 나누어 사람에게 배분하는 방식이었다. 한 사람이 혼자 할 수 없는 일이 조직 안에서 가능해졌고, 그 대가로 개인은 고유한 전체성 대신 분업의 한 조각으로 자신을 규정했다. 직업·직급·소속이 정체성을 대체했고, '회사 사람'이 근대적 인간의 표준형이 되었다.
        </p>
        <p style={ps}>
          AI 시대의 핵심은 자동화다. 분업의 각 조각을 기계가 점점 더 많이 수행하게 되면서, 그 조각들을 묶어두던 조직의 접착제가 풀린다. 조직은 여전히 존재하지만, 개인이 반드시 그 안에 있어야 할 이유가 줄어든다. 한 명이 AI와 결합해 과거 부서 하나가 하던 일을 해낼 수 있을 때, 개인은 조직에서 떨어져 나와 자신의 역량·관심·기여를 중심으로 삶을 재편할 수 있게 된다. 분업의 시대에 조직이 개인을 조립했다면, 자동화의 시대에 개인은 스스로 조립된다. 이 구조적 전환이 다음에 이어지는 모든 논의 — 역량 확장의 네 축, 공동체 재균형, 네 갈망 — 의 출발점이다.
        </p>
      </div>
      ...
      <div style={{ padding: "0 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28 }}>
        ...
        <div>
          ...
          <p style={ps}>
            AI가 인간을 대체할지 확장할지는 기술의 내재적 속성이 아니라 <strong>제도의 설계가 결정하는 문제</strong>다. 우리의 입장은, 접근·분배·규제의 조건이 적절히 갖춰질 때 AI는 인간을 대체하는 대신 인간의 역량을 확장한다는 것이다. 그 근거는 이미 관찰되는 현상에 있다. 한 사람이 과거 여러 직업군과 여러 조직이 나누어 맡던 역할을 동시에 겸하는 일이 가능해진다. 의사 한 명이 진료와 논문 집필, 의료 유튜브 채널과 헬스케어 앱 프로토타입까지 병행하고, 변호사 한 명이 소송을 맡으면서 법률 콘텐츠와 리걸테크 스타트업을 운영한다. 독립 연구자는 대학 없이 논문을 쓰고 강연하고 자문하며 창업하고, 한 명의 개발자는 수억 명이 쓰는 서비스를 만들면서 기관급 컴퓨팅 자원을 시간 단위로 임대한다. 능력의 양이 커지는 데서 그치지 않고, 한 사람이 운용할 수 있는 역할의 종류 자체가 늘어난다. 이 가능성이 '대체'가 아닌 '확장'으로 현실화되려면, ① AI 접근이 공공재로 보장되고, ② 분배 구조가 AI가 만든 부를 노동과 돌봄의 보완으로 환류시키며, ③ 규제가 독점을 견제하는 조건이 충족되어야 한다. 그 조건 아래에서 이 네 축의 동시 이전이 개인과 기관 사이의 역량 격차를 좁히고, 그만큼 더 큰 자유의 여지를 만든다.
          </p>
