...
function AreaTag({ areas }: { areas: string }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 700, color: C.accent, marginTop: 4 }}>
      관련 강화 영역: {areas}
    </div>
  );
}
      ...
      <div style={{ padding: "0 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, flex: 1 }}>
          ...
          {[
            {
              ...
              areas: "실행력",
              desc: "단일 직업에 자신을 귀속시키지 않고 다층 정체성을 살 권리. 시장 가치로 환원되지 않는 기여 — 돌봄·학습·창작·공동체 활동 — 가 사회적으로 인정받고, 그에 비례한 분배·존엄을 보장받을 권리. 묶음 직업·니치마켓 시대의 직업 재편 충격을 흡수하는 사회적 기반.",
            },
            {
              n: "②",
              title: "AI 인프라 평등 접근권",
            ...
            <div key={title} style={{ marginBottom: 10, padding: "10px 12px", borderLeft: `4px solid ${C.navy}`, background: C.bg }}>
              ...
              <AreaTag areas={areas} />
              <div style={{ fontSize: 12.5, lineHeight: 1.65, color: "#222", marginTop: 4 }}>{desc}</div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          {[
            ...
            {
              n: "②",
              title: "개발자 안전 입증 (FDA 모델) + 한시·미션 한정 권력",
            ...
            },
            {
              ...
              desc: "AI·연산·데이터·네트워크 같은 기반재는 사적 독점에서 공공재로 전환한다. 그리고 폭력 독점의 균열·기후·팬데믹처럼 어느 국가도 단독으로 통제할 수 없는 위기에 대해선 한시·미션 한정 글로벌 거버넌스(국제 AI 안전 기구, 디지털 권리 협약 등)가 작동한다 — 핵 군축이 우리에게 가르친 모델의 AI 적용.",
            },
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ background: C.navy, color: "#fff" }}>
              ...
              <th style={{ padding: "6px 8px", textAlign: "left", fontSize: 11.5, width: "19%" }}>디지털 위협</th>
              <th style={{ padding: "6px 8px", textAlign: "left", fontSize: 11.5 }}>핵심 처방 (5.1 권리 · 5.2 구조)</th>
            </tr>
          </thead>
          <tbody>
            {[
              { lv: "진행 중", urg: true, area: "판단력", threat: "폭력 독점의 균열", rx: "5.2-② FDA 모델 (개발자 안전 입증·사전 승인) + 5.2-③ 글로벌 한시 협력 (IASA·핵 군축 모델 AI 적용)" },
              { lv: "진행 중", urg: true, area: "파급력·판단력", threat: "허위정보·플랫폼 과두", rx: "5.1-③ 알고리즘 자기결정권 + 5.2-③ AI 인프라 공공재화로 빅테크 독점 해체" },
              { lv: "임박", urg: false, area: "실행력", threat: "직업 재편의 충격", rx: "5.1-① 정체성 실현권 + 재편 충격 완충 — 전환 인프라 구축, AI 이익 환수·환류, 시장 가치 너머 기여(돌봄·창작)에 분배·존엄 보장" },
              { lv: "임박", urg: false, area: "파급력", threat: "선거·공론장 AI 개입", rx: "5.1-③ 알고리즘 자기결정권 (설명 요구권·자동 판단 거부권) + 5.2-② FDA 모델로 고위험 시스템 사전 승인" },
              { lv: "중기", urg: false, area: "판단력", threat: "AI 정렬 문제", rx: "5.2-② FDA 모델 (개발자 안전 입증·외부 검증) + 5.2-③ 글로벌 한시 협력으로 통제 권한 확보" },
              { lv: "진행 중", urg: true, area: "실행력", threat: "미중 기술 패권 블록화", rx: "5.2-③ 글로벌 한시 협력 — 미·중·EU·글로벌 사우스 공동 의제로 디지털 거버넌스 끌어올리기" },
              { lv: "장기", urg: false, area: "판단력", threat: "범용 AI 통제 불능", rx: "5.2-② FDA 모델 + 5.2-③ 글로벌 한시 협력으로 확산 이전 통제 레짐 확보" },
              { lv: "장기", urg: false, area: "판단력·실행력", threat: "디지털 계급 고착·지속불가능성", rx: "5.1-② AI 인프라 평등 접근권 + 5.1-① 정체성 실현권 + 5.2-③ 공공 인프라로 환경·자원 사회화" },
            ].map((r, i) => (
              <tr key={i} style={{ background: i % 2 ? C.bg : "#fff" }}>
                ...
                <td style={{ padding: "5px 8px", borderBottom: "1px solid #eee", fontWeight: 700, color: "#222", lineHeight: 1.5 }}>{r.threat}</td>
