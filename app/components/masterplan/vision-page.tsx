    ...
    <div style={{ pageBreakAfter: "always", minHeight: "280mm" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, padding: "0 32px" }}>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, margin: "0 0 12px" }}>
            {[
              ["① AI 생물무기", "비전문가도 병원체 설계 — 임박한 위협"],
              ["② 사이버 WMD", "인프라 취약점 자동 탐색 — 임박한 위협"],
              ["③ 핵AI화·자율살상", "핵 지휘체계 AI 개입, 전쟁 성격 변화"],
              ["④ 감시·조작 체제", "민주주의를 안에서부터 침식"],
              ["⑤ AI 통제 상실", "에이전트가 인간 의도와 무관하게 행동"],
              ["⑥ 대량 실업·빅테크 독점", "경제적·존재론적 위기, CEO 10명이 80억 설계"],
            ].map(([t, d]) => (
              <div key={t} style={{ padding: "6px 8px", borderLeft: `3px solid ${C.navy}`, background: "#f8f9fa" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: C.navy, marginBottom: 2 }}>{t}</div>
                <div style={{ fontSize: 12, color: "#333", lineHeight: 1.5 }}>{d}</div>
              </div>
            ))}
          </div>
          <p style={ps}>
        ...
        <div>
          ...
          <p style={ps}>
            디지털 위협을 정확히 정의하고, 통제 방안을 설계하며, 경제 질서의 전환 모델을 제시한다. 국제 자문단과 함께 세계 수준의 이론적 토대를 만든다. 디지털 위협 통제는 세계적으로도 초기 단계 — 지금 시작하면 거차가 없다.
          </p>
        </div>
      </div>
