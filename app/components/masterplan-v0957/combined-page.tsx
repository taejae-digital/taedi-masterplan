...
const classRows: ClassRow[] = [
  {
    ...
    digital: {
      production: "여러 에이전트에 동시 위임 — 인간 협력자 + AI 에이전트로 구성된 팀 운영",
      distribution: "에이전트 인프라·데이터·권한 설계권 보유로 이익 집중 (과세 대상)",
      consumption: "에이전트가 대신 비교·계약·관리 — 시간을 사고 능력을 빌리는 소비",
    },
  },
  {
    label: "프로",
    desc: "전문 + 에이전트 확장 · 53→28%",
    industrial: {
    ...
    digital: {
      production: "자기 전문 영역 + 에이전트로 확장 영역 동시 수행 — 변호사가 회계도, 의사가 데이터 분석도",
      distribution: "전문성 프리미엄 축소(비전문가도 에이전트로 진입), 정체성 기여로 차별화",
      consumption: "에이전트 구독·도구 투자, 강소도시 맞춤 서비스",
    },
  },
  {
    label: "아마추어",
    desc: "에이전트로 능력 확장 · 35→60%",
    industrial: {
    ...
    digital: {
      production: "에이전트로 못 하던 일을 함 — 디자인·코딩·회계·번역을 비전문가가 직접, 1인이 회사 하나 수준의 산출",
      distribution: "디지털 배당 + 에이전트 활용 수익 (롱테일 상위만 큰 수익)",
      consumption: "에이전트 도구 구독, 정체성별 다양한 경로",
    },
  },
  {
    label: "소외",
    desc: "에이전트 접근권 부재 · 10→5%",
    industrial: {
    ...
    digital: {
      production: "에이전트 접근권 부재로 능력 확장에서 배제 — 격차 절대화",
      distribution: "안전망 + 공공 에이전트 접근권 보장으로 격차 차단",
      consumption: "기본 생존 + 정체성 발견·에이전트 활용 훈련 지원",
    },
  },
];
      ...
      <div style={{ padding: "8px 32px 16px", display: "flex", flexDirection: "column", flex: 1, gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11.5, color: "#222", lineHeight: 1.55, padding: "6px 10px", background: "#f0f4ff", borderLeft: `3px solid ${C.navy}`, marginBottom: 6 }}>
            <strong style={{ color: C.navy }}>산업시대 경제는 노동가치설 위에 섰다.</strong> 디지털시대는 <strong>에이전트가 능력을 확장</strong>한다 — 디자인·코딩·회계·번역을 비전문가가 직접 한다. 가치는 노동 시간에서 <strong>정체성 기여</strong>로 옮겨가고, 같은 4계층(리더·프로·아마추어·소외)이 산업시대와 디지털시대에 <strong>역할과 비중이 달라진다</strong>.
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "11%" }} />
            ...
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                <th rowSpan={2} style={{ padding: "5px 8px", textAlign: "left", fontSize: 11.5, borderRight: "1px solid rgba(255,255,255,0.2)", verticalAlign: "middle" }}>계층</th>
                <th colSpan={3} style={{ padding: "5px 8px", textAlign: "center", fontSize: 12, borderBottom: "1px solid rgba(255,255,255,0.3)", borderRight: "1px solid rgba(255,255,255,0.2)" }}>산업시대</th>
                <th colSpan={3} style={{ padding: "5px 8px", textAlign: "center", fontSize: 12, borderBottom: "1px solid rgba(255,255,255,0.3)", background: C.accent }}>디지털시대</th>
              </tr>
              <tr style={{ background: C.navy, color: "#fff" }}>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11 }}>생산</th>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11 }}>분배</th>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11, borderRight: "1px solid rgba(255,255,255,0.2)" }}>지출</th>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11, background: C.accent }}>생산</th>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11, background: C.accent }}>분배</th>
                <th style={{ padding: "5px 8px", textAlign: "left", fontSize: 11, background: C.accent }}>지출</th>
              </tr>
            </thead>
            <tbody>
              {classRows.map((r, i) => (
                <tr key={i} style={{ background: i % 2 ? C.bg : "#fff" }}>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 12, verticalAlign: "top", lineHeight: 1.45 }}>
                    <div style={{ fontWeight: 800, color: C.navy }}>{r.label}</div>
                    <div style={{ fontSize: 10.5, color: "#666", marginTop: 2 }}>{r.desc}</div>
                  </td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: "#444" }}>{r.industrial.production}</td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: "#444" }}>{r.industrial.distribution}</td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: "#444", borderRight: `2px solid ${C.navy}` }}>{r.industrial.consumption}</td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: C.navy, fontWeight: 500 }}>{r.digital.production}</td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: C.navy, fontWeight: 500 }}>{r.digital.distribution}</td>
                  <td style={{ padding: "8px 8px", borderBottom: "1px solid #eee", fontSize: 11, verticalAlign: "top", lineHeight: 1.5, color: C.navy, fontWeight: 500 }}>{r.digital.consumption}</td>
                </tr>
              ))}
            </tbody>
          </table>
