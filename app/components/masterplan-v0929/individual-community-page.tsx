...
export function IndividualCommunityPage() {
  ...
  const eras = [
    ...
    },
    {
      ...
      communityDetail: "(재결합)",
      task: "정체성대로 사는 삶",
      taskDetail: "남을 위한 노동 → 나를 위한 일",
      expertise: "깊이+너비 통합, AI 조율",
      ideal: "정체성 실현인",
      freedom: "자아실현의 자유",
      freedomDetail: "정체성 실현, 역할·기능·생계 강박 넘어",
      order: "★ 미정",
      orderDetail: "산업시대 질서가 더는 작동하지 않음",
      pending: true,
    },
  ];
  ...
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 32px", flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* 2.1 역사의 거대한 전환과 디지털 시대의 과제 */}
        <div>
          <div style={h3s}>2.1 역사의 거대한 전환과 디지털 시대의 과제</div>
          <div style={{ fontSize: 12.5, lineHeight: 1.5, color: "#222", marginBottom: 8 }}>
            인류는 세 번 자기 한계를 넘었다. 매번 새 기술이 새 공동체와 새 자유를 끌어냈다.
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 8 }}>
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                <th style={{ padding: "6px 8px", textAlign: "left", width: "14%", fontSize: 10.5, fontWeight: 600, letterSpacing: 1 }}>속성</th>
                {eras.map((e, i) => (
                  <th key={i} style={{ padding: "8px 10px", textAlign: "left", width: "28.66%", borderLeft: "1px solid rgba(255,255,255,0.2)" }}>
                    <div style={{ fontSize: 14, fontWeight: 800 }}>{e.name}</div>
                    <div style={{ fontSize: 10.5, opacity: 0.8, fontWeight: 500, marginTop: 1 }}>{e.sub}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {/* Row 1: 시대 본질 */}
              <tr style={{ borderBottom: `1px solid ${C.navy}` }}>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>시대 본질</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd`, color: e.pending ? C.red : "#222" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: e.pending ? C.red : C.navy }}>{e.essence}</div>
                    <div style={{ fontSize: 10.5, color: "#555", marginTop: 2, lineHeight: 1.4 }}>{e.essenceDetail}</div>
                  </td>
                ))}
              </tr>
              {/* Row 2: 핵심 기술 (산업은 3-stack) */}
              <tr style={{ borderBottom: `1px solid ${C.navy}` }}>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>핵심 기술</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd` }}>
                    {e.techType === "stack" ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        {(e.tech as Array<{n: string; label: string; era: string}>).map((t, k) => (
                          <div key={k} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${C.navy}`, padding: "3px 8px", fontSize: 11 }}>
                            <span style={{ fontWeight: 800, color: C.navy }}>{t.n}</span>
                            <span style={{ fontWeight: 700, color: "#222" }}>{t.label}</span>
                            <span style={{ fontSize: 9.5, color: "#888", marginLeft: "auto" }}>{t.era}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{ fontSize: 12, fontWeight: 700, color: "#222" }}>{(e.tech as string[])[0]}</div>
                    )}
                  </td>
                ))}
              </tr>
              {/* Row 3: 공동체 단위 */}
              <tr style={{ borderBottom: `1px solid ${C.navy}` }}>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>공동체 단위</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd` }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#222" }}>{e.community}</div>
                    <div style={{ fontSize: 10.5, color: "#888", marginTop: 1 }}>{e.communityDetail}</div>
                  </td>
                ))}
              </tr>
              {/* Row 4: 과제 / 전문성 / 인재상 */}
              <tr style={{ borderBottom: `1px solid ${C.navy}` }}>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>과제 · 전문성 · 인재상</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd` }}>
                    <div style={{ marginBottom: 5 }}>
                      <div style={{ fontSize: 9, color: "#888", fontWeight: 600, letterSpacing: 0.5 }}>과제</div>
                      <div style={{ fontSize: 11, color: "#222", fontWeight: 600, lineHeight: 1.4 }}>{e.task}</div>
                      {e.taskDetail && <div style={{ fontSize: 10, color: "#666", fontStyle: "italic" }}>{e.taskDetail}</div>}
                    </div>
                    <div style={{ marginBottom: 5 }}>
                      <div style={{ fontSize: 9, color: "#888", fontWeight: 600, letterSpacing: 0.5 }}>전문성</div>
                      <div style={{ fontSize: 11, color: "#222", lineHeight: 1.4 }}>{e.expertise}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 9, color: "#888", fontWeight: 600, letterSpacing: 0.5 }}>인재상</div>
                      <div style={{ fontSize: 11, color: e.pending ? C.red : "#222", fontWeight: 700, lineHeight: 1.4 }}>{e.ideal}</div>
                    </div>
                  </td>
                ))}
              </tr>
              {/* Row 5: 추구한 자유 */}
              <tr style={{ borderBottom: `1px solid ${C.navy}` }}>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>추구한 자유</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd` }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: e.pending ? C.red : C.navy, lineHeight: 1.35 }}>{e.freedom}</div>
                    <div style={{ fontSize: 10.5, color: "#555", marginTop: 2, lineHeight: 1.4 }}>{e.freedomDetail}</div>
                  </td>
                ))}
              </tr>
              {/* Row 6: 권력 · 경제 질서 (디지털 미정 강조) */}
              <tr>
                <td style={{ padding: "8px 10px", background: C.bg, fontWeight: 700, color: C.navy, fontSize: 10.5, verticalAlign: "top" }}>권력 · 경제 질서</td>
                {eras.map((e, i) => (
                  <td key={i} style={{ padding: "8px 10px", verticalAlign: "top", borderLeft: `1px solid #ddd`, background: e.pending ? "#fef2f2" : "transparent", borderTop: e.pending ? `2px dashed ${C.red}` : undefined, borderBottom: e.pending ? `2px dashed ${C.red}` : undefined, borderRight: e.pending ? `2px dashed ${C.red}` : undefined }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: e.pending ? C.red : C.navy, fontStyle: e.pending ? "italic" : "normal" }}>{e.order}</div>
                    <div style={{ fontSize: 10.5, color: e.pending ? C.red : "#555", marginTop: 2, lineHeight: 1.4, fontStyle: e.pending ? "italic" : "normal" }}>{e.orderDetail}</div>
                  </td>
                ))}
              </tr>
            </tbody>
          </table>
          <div style={{ padding: "8px 12px", background: "#fff4ec", borderLeft: `4px solid ${C.red}`, fontSize: 12, lineHeight: 1.55, color: "#222", fontStyle: "italic" }}>
            디지털 시대의 새 권력·경제 질서를 어떻게 설계할 것인가 — <strong style={{ color: C.red, fontStyle: "normal" }}>이 질문에 답하는 것이 본 연구의 출발점이다.</strong>
          </div>
        </div>
