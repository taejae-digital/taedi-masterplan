          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, marginBottom: 6 }}>
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                ...
                <th style={{ padding: "4px 6px", textAlign: "left", width: "12%" }}>이상적 인재상</th>
              ...
              {[
                {
                  ...
                  ageColor: false,
                  rows: [
                    { level: "리더", task: "위계 질서의 정당화", tech: "위계를 신성화한 종교", expert: "의례·통치의 권위", ideal: "지혜로운 자 / 원로(elder)", freedom: "이동의 자유 — 태어난 신분과 공간에 묶이지 않을 자유" },
                    { level: "프로", task: "세대 간 지식의 전승", tech: "기록을 가능케 한 문자", expert: "경험의 축적과 전승", ideal: "지혜로운 자 / 원로(elder)", freedom: "" },
                    { level: "아마추어", task: "식량 생산의 안정화", tech: "물을 다스리는 관개술", expert: "체화된 생산의 기예", ideal: "지혜로운 자 / 원로(elder)", freedom: "" },
                  ],
                },
                {
                  age: "산업",
                  ageColor: false,
                  rows: [
                    { level: "리더", task: "대규모 조직의 운영·자본 조달", tech: "분업을 제도화한 관료제·조립라인", expert: "생산 시스템을 소유하고 설계", ideal: "부품형 인간 / 숙련 노동자 / 전문가", freedom: "정체성의 자유 — 주어진 역할과 생계 강박을 넘어 내가 누구인지 스스로 정의할 자유" },
                    { level: "프로", task: "공간·시간의 압축·분과 전문화", tech: "거리를 좁힌 철도·전신", expert: "분과별 심화 전문성", ideal: "부품형 인간 / 숙련 노동자 / 전문가", freedom: "" },
                    { level: "아마추어", task: "인간 근력의 한계 극복", tech: "에너지를 생산하는 동력기관", expert: "정형화된 반복 노동", ideal: "부품형 인간 / 숙련 노동자 / 전문가", freedom: "" },
                  ],
                },
                {
                  age: "디지털",
                  ageColor: true,
                  rows: [
                    { level: "리더", task: "플랫폼·AI 거버넌스·가치 정렬 설계", tech: "AI·플랫폼 인프라·정렬 기술", expert: "기술이 추구할 목적과 생태계 규칙 설계", ideal: "T자형 인간 / 조율자(orchestrator)", freedom: "정체성 실현의 자유 — 잘하고, 좋아하고, 사회가 필요로 하는 일을 하는 자유" },
                    { level: "프로", task: "디지털 시스템 구축·AI 감독·제도 설계", tech: "소프트웨어·AI 감독·데이터 분석", expert: "기술과 창의를 결합한 가치 생산·비판적 평가", ideal: "T자형 인간 / 조율자(orchestrator)", freedom: "" },
                    { level: "아마추어", task: "디지털 도구·AI 활용·데이터 생산", tech: "생성 AI·플랫폼·SNS", expert: "AI와 협업하며 삶의 목적을 설계", ideal: "T자형 인간 / 조율자(orchestrator)", freedom: "" },
                  ],
                },
              ].flatMap(({ age, ageColor, rows }) => rows.map((r, j) => ({ ...r, age, ageColor, isFirst: j === 0, rowCount: rows.length }))).map(({ age, ageColor, isFirst, rowCount, level, task, tech, expert, ideal, freedom }, i) => (
                <tr key={i} style={{ background: ageColor ? "#f0f4ff" : i % 6 < 3 ? "#fff" : C.bg }}>
                  ...
                  {isFirst && <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, color: "#222", fontWeight: 500, lineHeight: 1.45, verticalAlign: "top", fontSize: 11.5 }}>{ideal}</td>}
                  {isFirst && <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, color: "#222", lineHeight: 1.45, verticalAlign: "top", fontWeight: 500, fontSize: 11 }}>{freedom}</td>}
                </tr>
              ))}
            </tbody>
          </table>
