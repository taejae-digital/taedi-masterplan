...
const ps: React.CSSProperties = { margin: "0 0 13px", fontSize: 13.8, lineHeight: 1.92, color: C.body, textAlign: "justify" };
...
const flow = [
  ...
  },
  {
    ...
    t: "정체성이 고유한 기여로 전환될 기본 여건을 보장한다",
  ...
  return (
    <div style={{ minHeight: "281mm", display: "flex", flexDirection: "column", background: "#fff" }}>
      ...
      <PageLead
        ...
        summary="AI와 로봇이 표준화된 일·단순 분석·반복 생산을 대체하면 남과 비슷하게 잘하는 능력의 가치는 떨어지고, 남과 다른 문제를 자기 방식으로 푸는 고유한 기여가 가치의 중심이 된다. 정체성은 감성적 자기표현이 아니라 AI 시대 생산성의 원천이며, 새 사회계약은 그 정체성이 공동체 안에서 충돌하지 않고 고유한 기여로 전환되도록 세계·국가·도시·가정 경영으로 실행한다."
      />
      ...
      <div style={{ padding: "6px 48px 0", display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: 42, rowGap: 18, flex: 1 }}>
        ...
        <div>
          <H3 no="1.3" title="경제·정치질서의 변화와 새로운 사회계약 — 정체성이 고유한 기여로 전환될 기본 여건을 보장한다" />
          <p style={ps}>
          ...
          <p style={ps}>
            <strong style={em}>AI 시대의 가치는 더 많이 아는 사람에게서 나오지 않는다.</strong> 누구나 지식과 도구에 접근할 수 있을 때, 가치는 무엇을 문제로 볼 것인지, 왜 그것을 풀 것인지, 어떤 방식으로 기여할 것인지에서 나온다. 이것이 정체성이 경제의 중심으로 이동하는 이유다. 문제는 이 다양성이 그냥 두면 협업이 아니라 파편화와 격차로 흩어진다는 점이다. AI·데이터·컴퓨팅 접근이 없거나, 플랫폼 추천에만 노출되거나, 시장과 연결되지 않으면 정체성은 고유한 기여로 이어지지 못한다. 새 사회계약은 개인의 자유를 줄이는 장치가 아니라, <strong style={em}>정체성이 고유한 기여로 전환될 여건</strong>을 공동체가 보장하는 약속이다. 대런 아세모글루(MIT 교수·2024 노벨경제학상)는 <em style={{ fontStyle: "normal", color: C.muted, fontWeight: 600 }}>『권력과 진보』(2023)에서 기술 진보의 향방은 정해진 운명이 아니라 권력과 제도의 선택이 결정한다고 본다.</em>
          </p>
        </div>
