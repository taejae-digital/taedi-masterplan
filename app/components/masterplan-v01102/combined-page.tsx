...
export function CombinedPage() {
  ...
  return (
    <div style={{ pageBreakBefore: "always", height: "281mm", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "10px 48px 8px", display: "flex", flexDirection: "column", flex: 1, gap: 8 }}> 
        <div style={{ paddingBottom: 6, borderBottom: `1px solid ${C.line}`, marginBottom: 0 }}>
          <div style={{ fontSize: 11, letterSpacing: 3, color: C.accent, fontWeight: 800, marginBottom: 5 }}>질서의 전환</div>
        ...
        <div style={{ marginTop: 4 }}>
          ...
          <SectionGrid spine={polSpine}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, height: "100%" }}>
              <div style={{ padding: "8px 12px", background: C.leadBlue, borderLeft: `3px solid ${C.accent}`, borderRadius: 5, fontSize: 12.3, lineHeight: 1.42, color: C.ink, fontWeight: 600 }}>
                디지털 시대에는 강화된 개인이 자기 정체성에 맞는 생산·학습·생활 조건을 요구하면서, 국가의 평균 규칙만으로는 정치적 필요를 담기 어려워진다. 생활정치와 정책 실험은 도시로 내려가고, AI·데이터·플랫폼·안보처럼 국경을 넘는 기술 권력은 세계정부의 강제력 있는 공통 규칙으로 올라간다. 국가는 두 질서를 연결하며 기본권·안전망·도시 간 격차를 조정하는 역할로 재편된다.
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
