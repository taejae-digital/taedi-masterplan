    ...
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "10px 32px 7px", borderBottom: `2px solid ${C.navy}`, marginBottom: 12 }}>
        <div style={{ fontSize: 12.2, color: "#222", lineHeight: 1.45, fontWeight: 600 }}>
          정치질서·경제질서, 세 권리, 사전승인 거버넌스, 국제 규율을 하나의 완결된 마스터플랜으로 묶고 외부 독자가 바로 이해할 수 있는 최종본을 만든다.
        </div>
      </div>
      ...
      <div style={{ padding: "0 32px 8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, flex: 1 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div>
            <div style={h3s}>5.4 업무 분장 — OKR별 책임 구조</div>
            <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", fontSize: 10.7 }}>
              <colgroup>
                <col style={{ width: "16%" }} />
                <col style={{ width: "17%" }} />
                <col style={{ width: "43%" }} />
              ...
              <thead>
                <tr style={{ background: C.navy, color: "#fff" }}>
                  <th style={assignTh}>담당</th>
                  <th style={assignTh}>연결 OKR</th>
                  <th style={assignTh}>핵심 책임</th>
                  <th style={assignTh}>산출물</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ["유호현", "O1 · O4", "v1.0 최종 논리 총괄. 정치질서·경제질서, 세 권리, 사전승인 거버넌스, 국제 규율을 하나의 사회계약 문장으로 통합한다.", "v1.0 최종본 · 외부 자문 질문지"],
                  ["김수연", "O1 · O2", "디지털 전환의 근거 자료와 사례를 수집한다. 경제질서, 4계층 재편, 정체성 실현의 실증 근거를 정리해 본문 판단을 뒷받침한다.", "근거 데이터팩 · 사례 메모"],
                  ["이규환", "O2 · O3", "AI 인프라 평등 접근권과 AI 개인화권을 정책·법제 언어로 구체화한다. 국가 운영, 데이터 주권, 공공 AI 접근, 설명·거부권을 연결한다.", "권리별 정책 초안 · 법제 비교표"],
                  ["윤준영", "O4", "사전승인 거버넌스와 국제 협력 축을 맡는다. 글로벌 네트워크, 다자협력 사례, 디지털 권력 구조 변동을 새 거버넌스 논리로 정리한다.", "국제 협력 맵 · 거버넌스 사례집"],
                ].map(([name, okr, duty, output], i) => (
                  <tr key={name} style={{ background: i % 2 ? C.bg : "#fff" }}>
                    <td style={{ ...assignTd, color: C.navy, fontWeight: 900 }}>{name}</td>
                    <td style={{ ...assignTd, color: C.accent, fontWeight: 900 }}>{okr}</td>
                    <td style={assignTd}>{duty}</td>
                    <td style={{ ...assignTd, fontWeight: 700, color: "#222" }}>{output}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div style={{ marginTop: 6, padding: "6px 8px", background: C.bg, borderLeft: `3px solid ${C.accent}`, fontSize: 10.7, lineHeight: 1.35, color: "#333", fontWeight: 600 }}>
              업무 분장은 사람별 활동 목록이 아니라 OKR 달성 책임이다. 각 담당 산출물은 5.3의 Key Results를 닫는 증거로 제출된다.
            </div>
          </div>
...
const assignTh: React.CSSProperties = { padding: "5px 7px", textAlign: "left", fontSize: 10.7, borderRight: "1px solid rgba(255,255,255,0.18)" };
const assignTd: React.CSSProperties = { padding: "5px 7px", borderBottom: "1px solid #eee", fontSize: 10.55, lineHeight: 1.3, color: "#333", verticalAlign: "top" };
