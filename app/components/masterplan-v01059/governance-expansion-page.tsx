...
const fearRows = [
  {
    actor: "미국",
    strength: "달러 금융·동맹망·에너지·대학·빅테크·군사력·글로벌 표준 설정·AI 법제 수출",
    shortFear: "중국 제조망과 글로벌 사우스 영향력이 커져 공급망·시장·동맹 결속이 흔들리는 것",
    longFear: "미국식 자유민주 질서와 달러·동맹 중심 패권이 약화되어 세계 규칙 설정권을 잃는 것",
    deal: "미국식 AI 인프라·법제를 동맹국 표준으로 확산하고, 중국식 무규칙 AI 확산을 막는 신뢰 기준 확보",
  },
  {
    actor: "중국",
...
function StrategyTitle({ no, area, title, compact = false }: { no: string; area: string; title: string; compact?: boolean }) {
  return (
    <div style={{ padding: compact ? "8px 12px 5px" : "9px 14px 6px", borderBottom: "1px solid #d9dee8", display: "flex", alignItems: "baseline", gap: 10, color: C.navy }}>
      <div style={{ fontSize: compact ? 15.2 : 16.2, fontWeight: 900 }}>{no}</div>
      <div style={{ fontSize: compact ? 15.2 : 16.2, fontWeight: 850 }}>{area}</div>
      <div style={{ fontSize: compact ? 12.1 : 12.7, fontWeight: 600, color: "#333" }}>{title}</div>
    </div>
  );
}
...
function WorldCard() {
  return (
    <section style={{ background: "#fff", display: "flex", flexDirection: "column", minHeight: 264 }}>
      ...
      <div style={{ padding: "7px 12px 8px", display: "flex", flexDirection: "column", gap: 7, flex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 9 }}>
          {fearRows.map((row) => (
            <div key={row.actor} style={{ border: `1px solid ${C.cardBorder}`, background: C.cardBg, display: "flex", flexDirection: "column" }}>
              <div style={{ background: C.cardHead, color: C.navy, fontSize: 12.5, fontWeight: 900, padding: "5px 8px", borderBottom: `1px solid ${C.cardBorder}` }}>{row.actor}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "17% 1fr", gap: 10, alignItems: "stretch" }}>
          ...
          <div style={{ background: C.leadBlue, borderLeft: `3px solid ${C.navy}`, padding: "7px 10px", fontSize: 10.25, lineHeight: 1.2, color: "#333", fontWeight: 560 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px" }}>
              ...
              <div><b style={{ color: C.navy }}>법제 경쟁</b> — 미국은 AI 규제 권한을 연방 중심으로 집중(주정부 10년 모라토리엄 추진)하고, EU는 AI Act로 권리 기반 규제를 확산시키며, 중국은 국가 주도 AI 거버넌스를 수출한다. 한국은 세 모델을 참조하되, 정체성이 고유한 기여로 전환될 여건을 보장하는 독자 모델로 포지셔닝한다.</div>
...
function StrategyCard({ section, compact = false }: { section: typeof sections[number]; compact?: boolean }) {
  if (section.no === "4.2") {
    return (
      <section style={{ background: "#fff", display: "flex", flexDirection: "column", minHeight: 244 }}>
        ...
        <div style={{ padding: "8px 14px 9px", display: "flex", flexDirection: "column", gap: 7, flex: 1 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 7 }}>
            {governanceAxes.map((axis) => (
              <div key={axis.label} style={{ border: `1px solid ${C.cardBorder}`, background: C.cardBg, display: "grid", gridTemplateColumns: "32px 1fr", minHeight: 92 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 7 }}>
            {nationalBelts.map((belt) => (
              <div key={belt.name} style={{ border: `1px solid ${C.cardBorder}`, background: "#fff", padding: "5px 6px", minHeight: 66 }}>
          ...
          <div style={{ background: C.cardHead, borderLeft: `3px solid ${C.navy}`, padding: "5px 8px", color: "#24364b", fontSize: 10.35, fontWeight: 760, lineHeight: 1.2 }}>
  ...
  return (
    <section style={{ background: "#fff", display: "flex", flexDirection: "column", minHeight: compact ? 208 : 202 }}>
      ...
      <div style={{ padding: compact ? "9px 12px 8px" : "10px 14px 8px", display: "grid", gridTemplateColumns: compact ? "1fr" : "30% 51% 19%", gap: compact ? 7 : 12, flex: 1 }}>
        ...
        <div style={{ background: C.leadBlue, borderLeft: compact ? "none" : `3px solid ${C.navy}`, borderTop: compact ? `3px solid ${C.navy}` : "none", padding: compact ? "7px 9px" : "8px 10px", color: C.navy, fontWeight: 850, fontSize: compact ? 10.8 : 11.0, lineHeight: 1.24 }}>{section.effect}</div>
      </div>
    </section>
  );
}
    ...
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "8px 36px 7px", borderBottom: `3px solid ${C.navy}`, marginBottom: 7 }}>
        <div style={{ fontSize: 19.1, color: C.navy, lineHeight: 1.22, fontWeight: 850, letterSpacing: -0.4 }}>
          디지털 시대에는 강화된 개인의 정체성이 공동체의 생산성으로 이어지도록 세계·국가·도시·가정의 운영 조건을 다시 설계해야 한다.
        </div>
        <div style={{ marginTop: 6, fontSize: 13.0, color: "#333", lineHeight: 1.28, fontWeight: 560 }}>
          새 사회계약은 선언만으로 실현되지 않는다. 세계는 안전한 경합의 규칙을 만들고, 국가는 學·産·市·媒와 산업벨트를 조직하며, 도시는 소도시-서버시티 벨트에서 생활권 실증을 수행하고, 가정은 개인 성장의 기반을 만든다.
        </div>
      </div>
