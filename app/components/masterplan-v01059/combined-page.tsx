    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 7, height: "100%" }}> 
      {rows.map((r, i) => (
        <div key={r.k} style={{ display: "grid", gridTemplateColumns: "84px 1fr", gap: 10, alignItems: "stretch", flex: 1 }}>
          ...
          <div style={{ padding: "7px 9px", background: i === rows.length - 1 ? "#e9efff" : i === 1 ? "#f1f5fb" : "#fff", border: "1px solid #d8deec" }}>
            <div style={{ fontSize: 12.95, color: C.navy, fontWeight: 850, lineHeight: 1.12, marginBottom: 3 }}>{r.t}</div>
            <div style={{ fontSize: 11.35, color: "#333", lineHeight: 1.18, fontWeight: 560 }}>{r.d}</div>
          </div>
        </div>
      ))}
    </div>
...
export function CombinedPage() {
  ...
  return (
    <div style={{ pageBreakBefore: "always", height: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "3px 36px 8px", display: "flex", flexDirection: "column", flex: 1, gap: 15 }}> 
        <div style={{ padding: "7px 18px 6px", borderBottom: `3px solid ${C.navy}`, marginBottom: 1 }}>
          <div style={{ fontSize: 18.2, color: C.navy, lineHeight: 1.18, fontWeight: 900, letterSpacing: -0.45 }}>
            디지털 시대에는 경제의 생산 주체가 개인으로 내려오고, 정치의 조정 권한은 도시와 세계정부로 재배치된다.
          </div>
          <div style={{ marginTop: 4, fontSize: 12.4, color: "#333", lineHeight: 1.24, fontWeight: 590 }}>
        ...
        <div>
          ...
          <SectionGrid spine={econSpine}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, height: "100%" }}> 
              <div style={{ padding: "5px 9px", background: "#f3f6fb", borderLeft: `3px solid ${C.navy}`, fontSize: 11.25, lineHeight: 1.2, color: "#2b3342", fontWeight: 650 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", marginTop: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "39% 61%", gap: 12, alignItems: "stretch" }}>
            <div style={{ background: C.navy, color: "#fff", padding: "13px 16px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ fontSize: 12.4, fontWeight: 850, opacity: 0.9, marginBottom: 6 }}>핵심 선언</div>
              <div style={{ fontSize: 24.0, lineHeight: 1.12, fontWeight: 950 }}>
                개인이 정체성을 발휘할 기본 여건을 보장한다.
              </div>
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: 7, justifyContent: "center" }}>
              <div style={{ fontSize: 13.0, lineHeight: 1.05, color: C.accent, fontWeight: 950, marginBottom: 1 }}>핵심 합의</div>
              {contractResponsibilities.map((r) => (
                <div key={r.title} style={{ display: "grid", gridTemplateColumns: "190px 1fr", alignItems: "stretch", border: "1px solid #dfe5ef", background: "#fff" }}>
                  <div style={{ background: "#e9efff", color: C.navy, padding: "9px 11px", fontSize: 12.6, lineHeight: 1.08, fontWeight: 950, display: "flex", alignItems: "center", borderRight: "1px solid #d6deef" }}>{r.title}</div>
                  <div style={{ padding: "9px 12px", fontSize: 14.1, lineHeight: 1.18, color: "#202a3a", fontWeight: 720 }}>{r.detail}</div>
                </div>
              ))}
            </div>
          </div>
...
const th: React.CSSProperties = { padding: "5.4px 7.5px", textAlign: "left", fontSize: 11.75, borderRight: "1px solid rgba(255,255,255,0.18)", verticalAlign: "middle", lineHeight: 1.16 };
...
const tdEcon: React.CSSProperties = { ...td, padding: "5.8px 7.5px", fontSize: 11.0, lineHeight: 1.2 };
...
const econHeadline: React.CSSProperties = { fontSize: 11.85, lineHeight: 1.12, fontWeight: 900, color: C.navy, marginBottom: 3 };
const econDetail: React.CSSProperties = { fontSize: 10.35, lineHeight: 1.18, fontWeight: 560, color: "#333" };
...
const th32: React.CSSProperties = { padding: "5.2px 7.5px", textAlign: "left", fontSize: 12.0, borderRight: "1px solid rgba(255,255,255,0.18)", verticalAlign: "middle", lineHeight: 1.1 };
const th32Mid: React.CSSProperties = { ...th32, background: "#1e3154" };
const td32: React.CSSProperties = { padding: "5.2px 7.5px", borderBottom: "1px solid #eee", fontSize: 11.15, verticalAlign: "middle", lineHeight: 1.14, color: "#333" };
...
const td32Label: React.CSSProperties = { ...td32, fontWeight: 850, color: C.navy, fontSize: 11.6 };
