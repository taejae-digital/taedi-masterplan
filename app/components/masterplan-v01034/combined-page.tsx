...
const contractExchangeRows = [
  { era: "농업시대", means: "토지", identity: "땅과의 관계로 정해짐", accept: "복종과 조세", guarantee: "생존과 보호" },
  { era: "산업시대", means: "자본·기계·분업", identity: "직업이 곧 정체성이 됨", accept: "노동과 법질서", guarantee: "임금·권리·복지" },
  { era: "디지털·AI 시대", means: "AI·데이터·컴퓨팅", identity: "목적·판단·고유한 기여의 패턴", accept: "플랫폼·알고리즘 질서", guarantee: "자기 생산 방향을 정하고 실험할 자유" },
];

const contractRows = [
  { label: "형성의 조건", problem: "정체성은 가정 자원·지역·초기 노출 격차와 플랫폼 추천에 의해 조기 분화", setting: "발견·훈련·전환 기회를 보완하고, 플랫폼의 정체성 조형 권력을 감시·조정" },
  { label: "발현의 조건", problem: "형성된 정체성이 생산과 기여로 이어지려면 AI·데이터·컴퓨팅 접근과 시장 연결이 필요", setting: "디지털 생산수단이 플랫폼과 자본에 집중되지 않도록 접근·실험·실패·연결 조건 보장" },
  { label: "인정의 조건", problem: "고유한 기여가 사회적으로 가치 있다고 확인되어야 정체성 기반 생산이 지속", setting: "플랫폼 노출·시장 성과·직함·학력이 인정 기준을 독점하지 않도록 복수의 인정 경로 설계" },
...
export function CombinedPage() {
  ...
  const contractSpine = [
    ...
    { k: "디지털 변화", t: "AI·데이터·컴퓨팅이 새 생산수단", d: "플랫폼과 알고리즘이 기회·노출·인정을 배열하고, 개인의 정체성 형성에도 개입" },
    { k: "새 질서", t: "정체성을 생산 방향으로 전환", d: "개인은 플랫폼 질서를 수용하는 대신, 자기 생산 방향을 정하고 실험할 자유를 보장받아야 함" },
  ];
  ...
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "270mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "9px 38px 9px", display: "flex", flexDirection: "column", flex: 1, gap: 9 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={h3s}>3.3 새로운 사회계약 — 정체성을 생산 방향으로 전환하는 운영 구조</div>
          <SectionGrid spine={contractSpine}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ padding: "4px 8px", background: "#f3f6fb", borderLeft: `3px solid ${C.navy}`, fontSize: 10.7, lineHeight: 1.18, color: "#2b3342", fontWeight: 650 }}>
                사회계약은 생산수단이 바뀔 때 다시 짜인다. 디지털·AI 시대에는 정체성이 목적·판단·고유한 기여의 패턴이 되며, 개인은 플랫폼·알고리즘 질서를 수용하는 대신 자기 생산 방향을 정하고 실험할 자유를 보장받아야 한다.
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                <colgroup><col style={{ width: "13%" }} /><col style={{ width: "18%" }} /><col style={{ width: "25%" }} /><col style={{ width: "20%" }} /><col style={{ width: "24%" }} /></colgroup>
                <thead><tr style={{ background: C.navy, color: "#fff" }}><th style={th33}>시대</th><th style={th33}>생산수단</th><th style={th33}>정체성</th><th style={th33}>개인이 수용한 것</th><th style={th33}>보장받은 것</th></tr></thead>
                <tbody>{contractExchangeRows.map((r, i) => <tr key={r.era} style={{ height: 31, background: i % 2 ? C.bg : "#fff", borderTop: i === 2 ? `2px solid ${C.navy}` : undefined }}><td style={td33Label}>{r.era}</td><td style={td33Blue}>{r.means}</td><td style={td33}>{r.identity}</td><td style={td33}>{r.accept}</td><td style={td33Strong}>{r.guarantee}</td></tr>)}</tbody>
              </table>
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                <colgroup><col style={{ width: "17%" }} /><col style={{ width: "41%" }} /><col style={{ width: "42%" }} /></colgroup>
                <thead><tr style={{ background: C.navy, color: "#fff" }}><th style={th33}>3층위</th><th style={th33}>방치하면 생기는 문제</th><th style={th33}>사회계약의 보장</th></tr></thead>
                <tbody>{contractRows.map((r, i) => <tr key={r.label} style={{ height: 34, background: i % 2 ? C.bg : "#fff" }}><td style={td33Label}>{r.label}</td><td style={td33}>{r.problem}</td><td style={td33Blue}>{r.setting}</td></tr>)}</tbody>
              </table>
            </div>
          </SectionGrid>
        </div>
