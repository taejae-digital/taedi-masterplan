...
const strategies = [
  ...
  },
  {
    ...
    strategy: "한반도의 산업, 인프라, 인재, 외교를 하나의 목표 아래 배치한다. 반도체·배터리·디스플레이·AI 인프라·바이오헬스·차세대 에너지·문화콘텐츠를 7대 초일류 분야로 묶고, 각 분야를 데이터 주권·AI 개인화권·안전망과 연결한다.",
    standard: "부처별 성장사업의 나열인가, 아니면 한국을 미래 질서의 필수 국가로 만드는 종합 운영 전략인가.",
  },
  {
    area: "도시 경영",
...
const fields = [
  ["반도체", "AI 연산과 군사·산업 공급망의 핵심 지렛대"],
  ["배터리", "전기화·모빌리티·분산 에너지 질서의 기반"],
  ["디스플레이", "디지털 경험과 문화 영향력의 접점"],
  ["AI 인프라", "공공 AI·데이터·컴퓨팅 접근권의 물적 조건"],
  ["바이오헬스", "개인 건강·돌봄·생애주기 데이터의 전략 산업"],
  ["차세대 에너지", "도시·산업·가정의 자립성과 회복력"],
  ["문화콘텐츠", "중국의 문명 흡수 전략에 맞서는 한국의 매력 자산"],
];
    ...
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "9px 32px 6px", borderBottom: `2px solid ${C.navy}`, marginBottom: 8 }}>
        <div style={{ fontSize: 12.4, color: "#222", lineHeight: 1.42, fontWeight: 650 }}>
          <strong style={{ color: C.navy }}>새로운 사회계약은 선언이 아니라 경영 전략이다.</strong> 세계는 한국을 필수불가결한 중간자로 배치해야 하고, 국가는 7대 초일류 분야를 하나의 목표로 묶어야 하며, 도시는 정체성 실현 환경이 되어야 하고, 가정은 AI 개인화권이 작동하는 생활 운영체계가 되어야 한다.
        </div>
      </div>
      ...
      <div style={{ padding: "0 32px 8px", display: "flex", flexDirection: "column", flex: 1, gap: 7 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.08fr 0.92fr", gap: 10, alignItems: "stretch" }}>
          <div>
            <div style={h3s}>4.3 한국의 7대 초일류 분야 — 위치적 독점의 재료</div>
        ...
        <div style={{ border: `1px solid ${C.navy}`, borderLeft: `6px solid ${C.accent}`, padding: "9px 15px", display: "flex", alignItems: "center", gap: 13 }}>
          ...
          <div style={{ fontSize: 15.2, lineHeight: 1.35, color: "#222", fontWeight: 700 }}>
            한국의 전략은 미국 선택과 중국 항복 사이의 선택지가 아니다. <span style={{ color: C.accent, fontWeight: 900 }}>새 사회계약을 세계·국가·도시·가정의 운영 원리로 만들고, 한국을 그 질서의 필수 연결점으로 세우는 것</span>이다.
          </div>
        </div>
      </div>
