...
const limitRows = [
  {
    title: "국제 질서",
    limit: "AI 패권경쟁은 시민 권리를 쉽게 진영 논리와 안보 통제 안에 가둔다. 새로운 사회계약이 작동하려면 디지털 인프라 접근과 시민 참여가 어느 진영의 플랫폼에 종속되지 않도록 국제 규칙이 필요하다.",
    rights: "디지털 인프라 접근권과 직접 거버넌스 참여권을 국제 검증·인증의 대상에 올린다.",
    strategy: "경쟁 중단을 요구하지 않는다. 군사·생물·사이버 악용 금지선, 공공 AI 감사 로그, 국제 사고 조사 절차처럼 패권국도 받아들일 수밖에 없는 안전장치를 먼저 설계한다.",
  },
  {
    title: "국가 전략",
    limit: "세 권리는 복지·참여·교육 사업으로 흩어지면 사회계약이 되지 못한다. 국가는 AI·데이터·산업·복지·정치 개혁을 하나의 운영 원리로 묶어야 한다.",
    rights: "디지털 인프라 접근권, 직접 거버넌스 참여권, 정체성 실현권을 법·예산·외교가 함께 움직이는 국가 전략으로 묶는다.",
    strategy: "한국은 AI 시민강화 규칙을 검증하고 확산하는 중심 국가가 되어야 한다. 15대 산업, 7대 분야 초일류, 선출직 인재를 따로 놓지 않고 하나의 사회계약 실행 체계로 연결한다.",
  },
  {
    title: "산업 기반",
    limit: "권리는 생활 속에서 작동해야 한다. AI 인프라, 도시·주거, 공공재, 로봇·드론, 미디어·금융, 교육·건강·국토가 연결될 때 시민은 실제로 더 높은 생산성과 선택권을 갖는다.",
    rights: "디지털 인프라 접근권이 일·학습·돌봄·공공서비스와 연결되어 정체성 실현권으로 이어진다.",
    strategy: "15대 산업은 먹거리 목록이 아니라 사회계약의 실증 기반이다. 미래창조형, 세계정상필수, 동북아·한반도 정체성기반 산업을 통해 시민강화 AI 운영 모델을 도시와 가정에서 증명한다.",
  },
  {
    title: "제도 운영",
    limit: "실증된 모델은 제도 권한으로 이어져야 한다. AI 안전, 데이터 주권, 디지털 배당, 시민 직접 참여가 법·예산·감사·행정 안에 들어오지 못하면 세 권리는 선언에 머문다.",
    rights: "직접 거버넌스 참여권이 법·예산·감사 권한을 갖고, 정체성 실현권이 교육·복지·산업 정책의 기준이 된다.",
    strategy: "싱크탱크·대학·기업·미디어·정부·정당·군대의 7대 분야를 초일류화한다. 이 제도들이 산업 실증을 규칙·신뢰·서사·권한으로 바꿀 때 사회계약이 실제 운영된다.",
  },
];
...
const strategyRows = [
  {
    area: "세계 경영",
    goal: "AI 패권경쟁의 공멸선을 검증 가능한 국제 규칙으로 설정한다.",
  ...
  {
    area: "도시 경영",
    goal: "시민강화 AI 운영 모델을 생활권에서 증명한다.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "아이·청년·중년·노년이 각자의 필요에 맞게 AI를 능력과 권리로 사용하는 생활 원칙 수립",
    ],
    ...
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "13px 36px 9px", borderBottom: `3px solid ${C.navy}`, marginBottom: 9 }}>
        <div style={{ fontSize: 19.2, color: C.navy, lineHeight: 1.25, fontWeight: 800, letterSpacing: -0.4 }}>
          한국은 AI 시민강화 규칙을 검증하고 확산하는 중심 국가가 되어야 한다.
        </div>
            ...
            {limitRows.map((row, i) => (
              <div key={row.title} style={{ minHeight: 245, display: "flex", flexDirection: "column", border: `1px solid #d8d2ca`, background: i % 2 ? C.bg : "#fff" }}>
                ...
                <div style={{ padding: "8px 10px 6px", fontSize: 11.15, lineHeight: 1.26, color: "#333", fontWeight: 520 }}>
                  <span style={{ color: C.navy, fontWeight: 750 }}>작동 조건</span> — {row.limit}
                </div>
                <div style={{ padding: "0 10px 7px", fontSize: 10.65, lineHeight: 1.23, color: C.navy, fontWeight: 700 }}>
                  <span style={{ color: C.accent, fontWeight: 850 }}>권리 연결</span> — {row.rights}
                </div>
