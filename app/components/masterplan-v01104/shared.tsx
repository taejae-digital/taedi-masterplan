...
export function Header({ subtitle, chapterNo, version = "v0.7" }: { subtitle: string; chapterNo?: string; version?: string }) {
  ...
  return (
    <div style={{ position: "relative", background: "#fff", color: C.ink, padding: "12px 48px 10px", overflow: "hidden", borderTop: `4px solid ${C.ink}`, borderBottom: `1px solid ${C.ink}` }}>
      <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
          {chapterNo && <span style={{ fontSize: 24, fontWeight: 900, color: C.ink, letterSpacing: -1, lineHeight: 1 }}>{chapterNo}</span>}
