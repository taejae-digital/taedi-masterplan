...
const governanceAxes = [
  { label: "學", title: "방향·지식·인재", actors: "싱크탱크 · 대학", body: "미래 의제·AI 거버넌스·산업/도시 전략을 설계하고 정체성·창업 역량을 기른다.", shift: "표준 인력 공급 → 정체성 기반 생산자를 기르는 지식 실험장" },
  { label: "産", title: "생산·시장·산업 벨트", actors: "기업 · 15대 산업", body: "AI·데이터·자본·시장·신뢰 인프라로 개인 생산자를 큰 시장에 연결한다.", shift: "대량생산 조직 → 개인 생산자의 시장 진입을 여는 산업 벨트 운영자" },
  { label: "政", title: "제도·정당성·안보", actors: "정부 · 정당 · 군대", body: "공공 데이터·조달·규제 샌드박스를 실행하고 입법·합의·안보를 보장한다.", shift: "국가 관리 → 생산 자유와 공동체 안전을 함께 보장하는 운영체계" },
  { label: "媒", title: "신뢰와 합의의 공론장", actors: "미디어", body: "정체성 생산·AI 위험·데이터 권리·산업 전환을 검증·토론할 의제로 만든다.", shift: "정보 매체 → 학·산·정을 사회적 합의로 바꾸는 공론장" },
];
...
const small = [
  {
    ...
    effect: "강소도시와 서버시티를 연결한 산업·생활 벨트가 전국에 분포하며 디지털 시대의 다양성과 생산성을 만든다. 리처드 플로리다(토론토대)는 인재와 창의 산업이 모이는 도시가 성장의 엔진이라고 본다.",
  },
  {
    no: "4.4", area: "디지털 시대의 가정 경영",
    title: "도시 안에서 탁월한 정체성이 발휘되는 시스템.",
    ...
    effect: "가정 경영은 보호 단위가 아니라 개인이 도시 생태계에서 탁월한 생산자로 성장하는 시스템이다.",
  },
];
...
function WorldSection() {
  ...
  return (
    <section>
      <SecHead no="4.1" area="디지털 시대의 세계 경영" desc="미·중 패권경쟁의 레드라인과 교환물을 설계한다." />
      <div style={{ color: C.body, fontSize: 12.5, lineHeight: 1.4, fontWeight: 600, marginBottom: 9 }}>
        미·중 경쟁이 WMD·자율살상무기·시장 분열로 번지지 않게 레드라인과 교환물을 설계하고, 한국은 두 나라가 모두 필요로 하는 AI 사회계약 실험장이 된다. <span style={{ color: C.muted, fontWeight: 600 }}>쇼샤나 주보프(하버드대)는 통제 안 된 데이터·플랫폼 권력이 개인의 자유를 잠식한다고 경고한다.</span>
      </div>

      {/* 미국 vs 중국 좌우 대치 */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 64px 1fr", alignItems: "stretch", columnGap: 0, marginBottom: 9 }}>
      ...
      {/* 협력적 딜 테이블 */}
      <div style={{ border: `1px solid ${C.cardBorder}`, borderRadius: 3, overflow: "hidden", marginBottom: 9 }}>
...
function NationSection() {
  return (
    <section>
      <SecHead no="4.2" area="디지털 시대의 국가 경영" desc="학·산·정·미디어 운영체계와 15대 산업벨트를 조직한다." />
      <div style={{ color: C.body, fontWeight: 600, fontSize: 12.5, lineHeight: 1.42, marginBottom: 10 }}>
        국가는 위에서 표준화하는 대신 學·産·政·媒 운영체계를 설계하고, 시산학 생태계는 도시가 맡는다. <span style={{ color: C.muted, fontWeight: 600 }}>마리아나 마추카토(UCL)는 국가가 위험을 감수하고 방향을 제시하는 미션 지향 주체가 되어야 한다고 본다.</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 9, marginBottom: 11 }}>
        {governanceAxes.map((axis) => (
          <div key={axis.label} style={{ border: `1px solid ${C.cardBorder}`, borderRadius: 3, overflow: "hidden", display: "grid", gridTemplateColumns: "34px 1fr" }}>
            <div style={{ background: C.ink, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, fontWeight: 900 }}>{axis.label}</div>
      ...
      <div style={{ color: C.ink, fontWeight: 800, fontSize: 12.5, marginBottom: 6 }}>15대 산업의 역할과 지역 배치 — 4대 산업 벨트</div>
      ...
      <div style={{ background: C.surface, borderLeft: `3px solid ${C.ink}`, borderRadius: 2, padding: "8px 13px", color: C.ink, fontSize: 11.5, fontWeight: 600, lineHeight: 1.45 }}>
        중부 벨트의 로봇·바이오·우주 원천기술이 동남권 벨트의 에너지·물류·드론으로 양산·운송되고, 서남권 벨트의 미래도시·공공재 실험장에서 생활 데이터로 검증된 뒤, 수도권·강원 벨트의 콘텐츠·금융·디자인을 거쳐 세계 시장으로 나간다. 시장의 수요·자본은 다시 중부의 연구로 되돌아온다. 어느 벨트도 혼자 완결되지 않고, 기술→양산→실증→시장→재투자의 순환이 4벨트를 한 산업 생태계로 묶는다.
      </div>
    </section>
  );
}
...
function SmallSection({ s }: { s: typeof small[number] }) {
  return (
    <section style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ border: `1px solid ${C.cardBorder}`, borderRadius: 3, padding: "10px 14px 11px", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ background: C.surface, borderLeft: `3px solid ${C.ink}`, borderRadius: 2, padding: "8px 12px", color: C.body, fontWeight: 600, fontSize: 11.4, lineHeight: 1.42 }}>{s.effect}</div>
      </div>
    </section>
  );
}
  ...
  return (
    <div style={{ pageBreakBefore: "always", height: "281mm", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <PageLead
        ...
        summary="새 사회계약은 선언만으로 실현되지 않는다. 세계는 안전한 경합의 규칙을 만들고, 국가는 學·産·政·媒와 산업벨트를 조직하며, 도시는 시산학 강소도시-서버시티 벨트에서 생활권 실증을 수행하고, 가정은 개인 성장의 기반을 만든다."
      />
      <div style={{ padding: "8px 44px 8px", display: "flex", flexDirection: "column", gap: 11 }}>
        <WorldSection />
        <NationSection />
