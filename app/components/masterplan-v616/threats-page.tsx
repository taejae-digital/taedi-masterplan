    ...
    <div style={{ pageBreakBefore: "always", pageBreakAfter: "always", height: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, flex: 1 }}>
        <div>
          ...
          <div style={{ padding: "7px 10px", background: "#fff8e1", borderLeft: `3px solid #f59e0b`, marginBottom: 10 }}>
          ...
          {/* 5차원 ↔ 통제 모델 연결 매핑 */}
          <div style={h3s}>강화 차원별 통제 모델 연결</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 4 }}>
            {[
              { dim: "판단력", arrow: "①", model: "개발자 입증 책임 (FDA 레짐)" },
              { dim: "생산성", arrow: "②", model: "역량·기여 기반 분배" },
              { dim: "범위", arrow: "③", model: "과업 한정 한시 위임 (WHO 모델)" },
              { dim: "접근성", arrow: "④", model: "플랫폼 공공성·데이터 권리" },
              { dim: "영향력", arrow: "⑤", model: "공론장 보호" },
            ].map(({ dim, arrow, model }) => (
              <div key={dim} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", background: C.bg, borderLeft: `2px solid ${C.navy}` }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: C.navy, minWidth: 52 }}>{dim}</div>
                <div style={{ fontSize: 11, color: C.light }}>→</div>
                <div style={{ fontSize: 11, color: "#333" }}><strong style={{ color: C.navy }}>{arrow}</strong> {model}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          {/* 4.3 통제 5 모델 */}
          <div style={h3s}>4.3 위협 통제의 5 모델</div>
          {[
            {
