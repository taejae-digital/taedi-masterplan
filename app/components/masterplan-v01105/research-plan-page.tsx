...
const futurePlan = [
  ...
  { t: "2028~2029", sub: "실증 사례화", items: ["강소도시 시범 (정체성 실현권 + 시민 직접 위임)", "사회계약 지표·피드백 체계 외부 협력 구현", "디지털 권리 헌장 초안 (세 권리 명문화)"] },
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          <div>
            ...
            <div style={{ padding: "6px 8px", background: C.bg, borderLeft: `3px solid ${C.navy}` }}>
              <div style={{ fontSize: 12.8, color: "#222", lineHeight: 1.5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              {[
                ...
                <div key={t} style={{ padding: "2px 0 4px" }}>
                  ...
                  <div style={{ fontSize: 11.6, lineHeight: 1.45, color: "#333" }}>{d}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {objectives.map(({ o, no, owner, advisor, krs }) => (
                <div key={no} style={{ border: `1px solid ${C.cardBorder}`, borderRadius: 3, overflow: "hidden", background: "#fff" }}>
                  ...
                  <div style={{ padding: "5px 9px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2.5px 10px" }}>
                    {krs.map((kr, i) => (
                      <div key={i} style={{ display: "flex", gap: 5, fontSize: 11, lineHeight: 1.4, color: "#333" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {futurePlan.map(({ t, sub, items }, i) => (
                  <div key={t} style={{ position: "relative", paddingLeft: 24 }}>
                    ...
                    <div style={{ border: `1px solid ${C.cardBorder}`, borderLeft: `3px solid ${C.ink}`, borderRadius: 3, background: C.bg, padding: "6px 11px" }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 4 }}>
                        <span style={{ fontSize: 14, fontWeight: 900, color: C.ink }}>{t}</span>
                        <span style={{ fontSize: 12, fontWeight: 800, color: C.navy }}>{sub}</span>
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "3px 0" }}>
                        {items.map((item, j) => (
                          <div key={j} style={{ display: "flex", gap: 5, width: "100%", fontSize: 11.4, lineHeight: 1.5, color: "#333" }}>
