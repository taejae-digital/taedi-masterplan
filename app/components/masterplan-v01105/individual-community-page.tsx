...
export function IndividualCommunityPage() {
  ...
  function EraTable() {
    ...
    const body: React.CSSProperties = { fontSize: 11.8, color: C.ink, fontWeight: 650 };
    const detail: React.CSSProperties = { fontSize: 10.2, color: C.muted, marginTop: 2, lineHeight: 1.32 };

    return (
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", fontSize: 12.0, borderTop: `1px solid ${C.ink}` }}>
        <colgroup>
          <col style={{ width: "10%" }} />
          <col style={{ width: "13%" }} />
          <col style={{ width: "17%" }} />
    ...
    <div style={{ pageBreakBefore: "always", height: "281mm", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 48px 0", flex: 1, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: "0 0 auto" }}>
          <div style={{ ...h3s, fontSize: 17.2, margin: "8px 0 7px" }}>2.2 가정에서 세계까지 — 공동체 6단계의 변화</div>
              ...
              <div key={i} style={{ display: "flex", alignItems: "stretch" }}>
                <div style={{ flex: 1, padding: "0 4px", display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ flex: 1, padding: "9px 10px 10px", background: i % 2 ? C.bg : "#fff", border: `1px solid ${C.cardBorder}`, borderTop: `3px solid ${bridgeColor}`, borderRadius: 3, display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: 14.2, fontWeight: 850, color: bridgeColor, lineHeight: 1.2, marginBottom: 6, paddingBottom: 6, borderBottom: `1px dashed ${C.cardBorder}` }}>{to}</div>
                    <div style={{ fontSize: 10.5, fontWeight: 850, color: C.muted, letterSpacing: 0.4, marginBottom: 4 }}>왜 필요한가</div>
                    <ul style={{ margin: "0 0 7px 0", paddingLeft: 13, color: C.muted, fontSize: 11.8, lineHeight: 1.27 }}>
                      {whyPoints.map((point, k) => <li key={k} style={{ marginBottom: 2 }}>{point}</li>)}
                    </ul>
                    <div style={{ fontSize: 10.5, fontWeight: 850, color: C.ink, letterSpacing: 0.4, marginBottom: 4, paddingTop: 5, borderTop: `1px solid ${C.hairline}` }}>새 역할</div>
                    <ul style={{ margin: 0, paddingLeft: 13, color: C.ink, fontSize: 12.2, lineHeight: 1.27, fontWeight: 500 }}>
                      {rolePoints.map((point, k) => <li key={k} style={{ marginBottom: 2 }}>{point}</li>)}
                    </ul>
                  </div>
                </div>
                {i < stages.length - 1 && (
