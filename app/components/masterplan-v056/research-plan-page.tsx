  ...
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "280mm" }}>
      ...
      <div style={{ padding: "0 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        {/* 왼쪽: 원리 + 연구 분야 */}
        <div>
          <SectionTitle>해결의 네 원리</SectionTitle>
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 7, marginBottom: 14 }}>
            {[
              { n: "①", t: "자기해체하는 권력 (Sunset Power)", d: "명시적 목표와 종료 조건, 외부 검증, 개인 이해관계와 기관 존속의 분리, 목표 달성 시 자동 해체 프로토콜. 권력은 과업에 종속되며, 과업이 끝나면 권력도 사라진다.", color: C.navy },
              { n: "②", t: "개발자 입증 책임 (FDA 레짐)", d: "AI 시스템을 출시하려는 자가 안전을 스스로 증명하고 사전 승인을 받아야 한다. 위험을 가장 잘 아는 쪽이 개발자이므로 입증 책임도 그쪽에 귀속.", color: C.red },
              { n: "③", t: "목적 한정 글로벌 위임 (WHO 모델)", d: "AI·팬데믹·기후 등 글로벌 위기에만 한정된 권한이 국가들로부터 위임된다. 위기 종료 시 자동 소멸. 만능 세계정부가 아닌 기능적 거버넌스.", color: C.amber },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 7, marginBottom: 14 }}>
            {[
              { t: "A. 새로운 경제·정치 질서의 이론 정립", d: "자본주의도 공산주의도 아닌 정보화 질서의 이론적 뼈대. Polanyi·Mazzucato·Acemoglu를 AI 시대에 재해석. 공공 AI 인프라·참여 기반 분배의 논리 구축.", color: C.navy },
              { t: "B. 폭력의 민주화 통제 체계", d: "AI 자율살상무기(LAWS)·AI·사이버 WMD의 국제 레짐. 개발자 입증 책임 레짐과 사전 승인 체계 설계. 미중 대타협의 가능 경로 연구.", color: C.red },
              { t: "C. 인간 존엄과 공동체 재설계", d: "프로 계층 정체성 붕괴 대응. 직업=정체성 공식을 대체할 정체성 실현권. 가정·마을·도시 단위 재연결과 돌봄 인정 체계.", color: C.accent },
              { t: "D. 빅테크 통제와 디지털 권리", d: "플랫폼 과두제 해체. 데이터 주권·알고리즘 투명성·디지털 권리 헌장. AI 인프라의 공공재화와 환경 영향 규제.", color: C.amber },
            ].map(({ t, d, color }) => (
              <div key={t} style={{ padding: "7px 10px", border: `1px solid ${C.line}`, borderLeft: `4px solid ${color}`, background: "#fff" }}>
                <div style={{ fontSize: 12.5, fontWeight: 800, color, marginBottom: 3 }}>{t}</div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 7, marginBottom: 12 }}>
            {[
              { name: "유호현 (팀장)", scope: "총괄 · 공공 AI 인프라 · 경제 질서 이론", focus: "연구 전체 방향 설정, 회장님·원장님과 소통, 공공 AI 인프라 및 역량 기반 분배 모델 구축, 미중 대타협 경로 연구, 단행본 편집 총괄.", color: C.navy },
              { name: "윤준영", scope: "기술·국제 · AI 자율살상무기 + 미중 패권", focus: "LAWS·AI WMD 현황과 국제 레짐 선례 분석, 개발자 입증 책임 체계 설계, 기술 공급망 기반 수출통제 검토, Bengio·ARIA 등과의 연결.", color: C.red },
              { name: "김수연", scope: "사회·공동체 · 인간 존엄 + 분배·노동", focus: "프로 계층 정체성 붕괴 대응, 직업=정체성 공식을 대체할 정체성 실현권, 6공동체 재연결, 돌봄·학습·창작 기여의 인정 구조 설계.", color: C.accent },
              { name: "이규환", scope: "제도·환경 · 빅테크 통제 + 지속불가능성", focus: "플랫폼 과두제 규율, 데이터 주권·알고리즘 투명성 법제, AI 데이터센터 전력·용수·탄소 영향 분석, 디지털 권리 헌장 초안.", color: C.amber },
            ].map(({ name, scope, focus, color }) => (
              <div key={name} style={{ padding: "8px 11px", borderTop: `3px solid ${color}`, background: C.bg }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 3 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 800, color }}>{name}</div>
