...
export function ResearchPlanPage() {
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 32px 12px", borderBottom: `1px solid ${C.bg}`, marginBottom: 12 }}>
        <div style={{ marginBottom: 6 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: C.navy, borderBottom: `2px solid ${C.navy}`, paddingBottom: 2 }}>AI 시대의 연구 방법론</div>
          <div style={{ fontSize: 11.5, color: "#444", marginTop: 4 }}>정보→지식 변환은 AI에 위임하고, 지식→지혜(방향 판단)에 인간 역량을 집중한다.</div>
        </div>
        <div style={{ display: "flex", alignItems: "stretch", gap: 6 }}>
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", flex: 1 }}>
              <div style={{ flex: 1, padding: "7px 9px", background: bg, borderTop: `3px solid ${color}` }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 2 }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: color }}>{num}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#111" }}>{t}</span>
                </div>
                <div style={{ fontSize: 10, color: color, fontWeight: 600, marginBottom: 3 }}>{who}</div>
                <div style={{ fontSize: 10.5, color: "#444", lineHeight: 1.4 }}>{d}</div>
              </div>
              {i < arr.length - 1 && (
                <div style={{ fontSize: 14, color: C.navy, fontWeight: 800, padding: "0 3px" }}>→</div>
              )}
            </div>
          ))}
        </div>
