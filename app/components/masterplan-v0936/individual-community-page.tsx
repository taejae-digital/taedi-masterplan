            ...
            {stages.map(({ label, to, bridgeRole, bridgeColor, why, def }, i) => (
              <div key={i} style={{ display: "flex" }}>
                <div style={{ flex: 1, padding: "14px 13px", background: i % 2 ? C.bg : "#fff", borderTop: `3px solid ${bridgeColor}`, borderBottom: `3px solid ${bridgeColor}`, display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#444", lineHeight: 1.55, marginBottom: 22, fontStyle: "italic", flex: 1 }}>{why}</div>
                  <div style={{ fontSize: 10, color: C.navy, fontWeight: 700, marginBottom: 3, textTransform: "uppercase", letterSpacing: 0.5 }}>새 역할</div>
                  <div style={{ fontSize: 11.5, color: "#111", lineHeight: 1.5, fontWeight: 600 }}>{def}</div>
                </div>
                {i < stages.length - 1 && (
