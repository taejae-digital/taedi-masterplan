...
const limitRows = [
  {
    ...
    strategy: "한국은 경쟁 중단을 요구하지 않는다. AI 상호절제·시민강화 협정, AI-FDA식 국제 검증 체계, WMD 감축 의제를 묶어 시민을 파괴하지 않는 공멸선을 세계 표준으로 제안한다.",
  },
  {
    title: "한국의 위치",
    ...
    strategy: "15대 산업은 물적 기반, 7대 분야 초일류는 제도 권력, 선출직 인재는 실행 권한이다. 이 셋을 법·예산·외교·산업정책으로 묶어 AI 시민강화 표준 패권을 만든다.",
  },
  {
    title: "거대도시의 비효율성",
    key: "세계 표준은 도시의 생활 증거로 만들어진다.",
    blocked: [
      "거대도시는 주거·일자리·교육·돌봄·행정·지역 산업을 분리한다.",
      "접근권은 장비 보급, 참여권은 앱 기능에 갇힌다.",
      "정체성 실현권은 상담 프로그램에 머물고 생활 증거가 없다.",
    ],
    strategy: "강소도시를 시민강화 AI 실험장으로 만든다. 학습·건강·돌봄·행정 경험, 개인 성장 기록, 대학·기업·공공조달·시민참여를 연결해 세계가 따라올 도시 모델을 증명한다.",
  },
  {
    title: "몰개인화",
    ...
    strategy: "가정은 가장 작은 사회계약 단위다. 정체성 발견, 전문성 훈련, 재교육, 노년의 판단 주도권, 건강·학습·돌봄 데이터 주권이 매일 작동해야 시민강화가 현실이 된다.",
  },
];
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
            {limitRows.map((row, i) => (
              <div key={row.title} style={{ minHeight: 147, display: "grid", gridTemplateColumns: "108px 1fr", border: `1px solid #d7d0c7`, background: i % 2 ? C.bg : "#fff", boxShadow: "0 1px 0 rgba(0,0,0,0.03)" }}>
                <div style={{ background: "#f2eee6", borderRight: `1px solid #d7d0c7`, padding: "10px 11px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div style={{ color: C.navy, fontSize: 15.2, fontWeight: 820, letterSpacing: -0.3 }}>{row.title}</div>
                  <div style={{ color: C.accent, fontSize: 10.2, fontWeight: 760, letterSpacing: 1.1 }}>전략 조건</div>
                </div>
                <div style={{ padding: "9px 12px 9px", display: "grid", gridTemplateColumns: "1fr 0.78fr", gap: 12 }}>
                  ...
                  <div style={{ borderLeft: "3px solid #d9b46f", paddingLeft: 10, fontSize: 10.8, lineHeight: 1.24, color: "#2f2f2f", fontWeight: 520 }}>
