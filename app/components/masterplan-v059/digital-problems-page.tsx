    ...
    <div style={{ pageBreakBefore: "always", pageBreakAfter: "always", minHeight: "280mm" }}>
      ...
      <div style={{ padding: "0 32px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 14 }}>
          {/* 왼쪽: 위험 지도 (2D 매트릭스) */}
          <div>
            <div style={h3s}>위험 지도 — 시급성 × 비가역성</div>
            <div style={{ display: "flex", marginBottom: 8, alignItems: "stretch" }}>
              {/* 세로 Y축 라벨 (바깥쪽) */}
              <div style={{ width: 22, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <div style={{ transform: "rotate(-90deg)", fontSize: 10.5, fontWeight: 800, color: C.red, whiteSpace: "nowrap" }}>비가역·공멸성 →</div>
              </div>

              <div style={{ position: "relative", flex: 1, height: 360, border: `1px solid ${C.line}`, background: "#fff" }}>
                ...
                {[
                  ...
                  <div key={label} style={{ position: "absolute", top: 4, left: `${pct}%`, transform: "translateX(-50%)", fontSize: 10, fontWeight: 700, color: "#888", background: "rgba(255,255,255,0.95)", padding: "0 4px" }}>{label}</div>
                ))}

                {/* 위험 노드 — 원 크기는 파급 범위(개인<도시<국가<세계) */}
                {[
                  { x: 92, y: 92, label: "폭력의 민주화", color: C.red, size: 26, lx: -8, ly: 22 },
                  { x: 86, y: 58, label: "허위정보·플랫폼·권위주의", color: C.red, size: 26, lx: -10, ly: 18 },
                  { x: 92, y: 28, label: "감시 자본주의", color: C.amber, size: 26, lx: -8, ly: 18 },
                  { x: 66, y: 40, label: "자동화 실업", color: C.amber, size: 20, lx: 14, ly: 14 },
                  { x: 58, y: 64, label: "선거 AI 개입", color: C.amber, size: 14, lx: 12, ly: -14 },
                  { x: 38, y: 88, label: "AI 정렬 문제", color: C.accent, size: 26, lx: 16, ly: 16 },
                  { x: 46, y: 46, label: "프로 계층 정체성", color: C.accent, size: 20, lx: 14, ly: 14 },
                  { x: 30, y: 68, label: "미중 블록화", color: C.accent, size: 26, lx: 16, ly: -14 },
                  { x: 12, y: 94, label: "AGI 통제 불능", color: C.navy, size: 26, lx: 16, ly: 18 },
                  { x: 18, y: 70, label: "디지털 계급 고착", color: C.navy, size: 26, lx: 16, ly: 16 },
                  { x: 8, y: 48, label: "지속불가능성", color: C.green, size: 26, lx: 16, ly: 16 },
                ].map(({ x, y, label, color, size, lx, ly }) => (
                  <div key={label} style={{ position: "absolute", left: `${x}%`, bottom: `${y}%`, pointerEvents: "none" }}>
                    <div style={{ width: size, height: size, borderRadius: "50%", background: color, border: `2px solid #fff`, boxShadow: `0 0 0 1.5px ${color}`, opacity: 0.5, position: "absolute", left: -size/2, top: -size/2 }} />
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: color, position: "absolute", left: -3, top: -3 }} />
                    <div style={{ position: "absolute", left: lx, top: ly - 7, fontSize: 10.5, fontWeight: 700, color: "#222", background: "rgba(255,255,255,0.97)", padding: "1px 4px", whiteSpace: "nowrap", transform: lx < 0 ? "translateX(-100%)" : undefined, border: `1px solid ${color}55` }}>{label}</div>
                  </div>
                ))}

                {/* X축 "시급성 →" (차트 내부 우하단) */}
                <div style={{ position: "absolute", right: 6, bottom: 4, fontSize: 10, fontWeight: 800, color: C.red, background: "rgba(255,255,255,0.95)", padding: "0 3px" }}>시급성 →</div>
              </div>
            </div>

            {/* 범례 + 설명 */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", fontSize: 10.5, color: "#333", marginBottom: 6 }}>
              <span style={{ fontWeight: 800, color: C.navy }}>원 크기 = 파급 범위</span>
              {[
                { label: "개인", size: 10 },
                { label: "도시", size: 14 },
                { label: "국가", size: 20 },
                ...
                <span key={label} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                  <span style={{ width: size, height: size, borderRadius: "50%", background: C.light, opacity: 0.5, display: "inline-block" }} />
                  <span>{label}</span>
                </span>
              ))}
            </div>
            <div style={{ fontSize: 10.5, color: C.light, lineHeight: 1.55 }}>
              우상단일수록 <strong style={{ color: C.red }}>지금 최대 위협</strong> — 시급성과 비가역성이 모두 높은 영역. 좌상단은 시간 여유는 있으나 실현 시 돌이킬 수 없는 위협. 우하단은 이미 진행중이나 사회적 조정으로 복구 가능한 위협.
            </div>
          </div>
