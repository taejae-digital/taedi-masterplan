...
export function IndividualCommunityPage() {
  ...
  const eras = [
    ...
    },
    {
      ...
      orderDetail: "민주주의·자본주의를 넘는 새 사회계약 — 정체성 실현권, AI 인프라 평등 접근권, 세계 시민 거버넌스",
      pending: false,
    },
  ];
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, marginBottom: 8 }}>
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "9%", fontSize: 11 }}>시대</th>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "13%", fontSize: 11 }}>시대 본질</th>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "17%", fontSize: 11 }}>핵심 기술</th>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "13%", fontSize: 11 }}>공동체 단위</th>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "18%", fontSize: 11 }}>과제 · 전문성 · 인재상</th>
                <th style={{ padding: "8px 10px", textAlign: "left", width: "15%", fontSize: 11 }}>추구한 자유</th>
                <th style={{ padding: "8px 10px", textAlign: "left", fontSize: 11 }}>권력 · 경제 질서</th>
              </tr>
            </thead>
            <tbody>
              {eras.map((e, i) => (
                <tr key={i} style={{ background: e.pending ? "#fff4ec" : i % 2 === 0 ? "#fff" : C.bg, borderBottom: `1px solid ${C.navy}` }}>
                  <td style={{ padding: "10px 10px", verticalAlign: "top" }}>
                    <div style={{ fontSize: 14, fontWeight: 800, color: C.navy }}>{e.name}</div>
                    <div style={{ fontSize: 10, color: "#666", marginTop: 1 }}>{e.sub}</div>
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top", lineHeight: 1.45 }}>
                    <div style={{ fontSize: 11.5, fontWeight: 700, color: e.pending ? C.red : C.navy }}>{e.essence}</div>
                    <div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{e.essenceDetail}</div>
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top" }}>
                    {e.techType === "stack" ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        {(e.tech as Array<{n: string; label: string; era: string}>).map((t, k) => (
                          <div key={k} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10.5 }}>
                            ...
                            <span style={{ fontSize: 9, color: "#888" }}>({t.era})</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{ fontSize: 11.5, fontWeight: 600, color: "#222" }}>{(e.tech as string[])[0]}</div>
                    )}
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top", lineHeight: 1.45 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: "#222" }}>{e.community}</div>
                    <div style={{ fontSize: 9.5, color: "#888", marginTop: 1 }}>{e.communityDetail}</div>
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top", lineHeight: 1.4 }}>
                    <div style={{ marginBottom: 4 }}>
                      <span style={{ fontSize: 8.5, color: "#888", fontWeight: 700, letterSpacing: 0.5, marginRight: 4 }}>과제</span>
                      <span style={{ fontSize: 10.5, color: "#222", fontWeight: 600 }}>{e.task}</span>
                      {e.taskDetail && <div style={{ fontSize: 9.5, color: "#666", fontStyle: "italic", marginLeft: 24, marginTop: 1 }}>{e.taskDetail}</div>}
                    </div>
                    <div style={{ marginBottom: 4 }}>
                      <span style={{ fontSize: 8.5, color: "#888", fontWeight: 700, letterSpacing: 0.5, marginRight: 4 }}>전문성</span>
                      <span style={{ fontSize: 10.5, color: "#222" }}>{e.expertise}</span>
                    </div>
                    <div>
                      <span style={{ fontSize: 8.5, color: "#888", fontWeight: 700, letterSpacing: 0.5, marginRight: 4 }}>인재상</span>
                      <span style={{ fontSize: 10.5, color: e.pending ? C.red : "#222", fontWeight: 700 }}>{e.ideal}</span>
                    </div>
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top", lineHeight: 1.45 }}>
                    <div style={{ fontSize: 11.5, fontWeight: 800, color: e.pending ? C.red : C.navy, lineHeight: 1.35 }}>{e.freedom}</div>
                    <div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{e.freedomDetail}</div>
                  </td>
                  <td style={{ padding: "10px 10px", verticalAlign: "top", lineHeight: 1.45, background: e.pending ? "#fef2f2" : "transparent", border: e.pending ? `2px dashed ${C.red}` : undefined }}>
                    <div style={{ fontSize: 11.5, fontWeight: 800, color: e.pending ? C.red : C.navy, fontStyle: e.pending ? "italic" : "normal" }}>{e.order}</div>
                    <div style={{ fontSize: 10, color: e.pending ? C.red : "#555", marginTop: 2, fontStyle: e.pending ? "italic" : "normal" }}>{e.orderDetail}</div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ padding: "8px 12px", background: "#fff4ec", borderLeft: `4px solid ${C.accent}`, fontSize: 12, lineHeight: 1.55, color: "#222" }}>
            <strong style={{ color: C.accent }}>디지털 시대의 새 질서는 민주주의·자본주의를 넘는 새 사회계약을 요구한다 —</strong> 정체성 실현권, AI 인프라 평등 접근권, 그리고 세계 시민 거버넌스. <strong style={{ color: C.navy }}>그 설계가 본 연구의 출발점이다.</strong>
          </div>
        </div>
