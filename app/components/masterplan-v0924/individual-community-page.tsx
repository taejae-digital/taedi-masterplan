  ...
  const ageGroups = [
    {
      ...
      ageColor: false,
      rows: [
        { level: "리더", task: "위계 질서의 정당화", tech: "위계를 신성화한 종교", expert: "의례·통치의 권위" },
        { level: "프로", task: "세대 간 지식의 전승", tech: "기록을 가능케 한 문자", expert: "경험의 축적과 전승" },
        { level: "아마추어", task: "식량 생산의 안정화", tech: "물을 다스리는 관개술", expert: "체화된 생산의 기예" },
      ],
    ...
    },
    {
      ...
      ageColor: false,
      rows: [
        { level: "리더", task: "대규모 조직의 운영·자본 조달", tech: "분업을 제도화한 관료제·조립라인", expert: "생산 시스템을 소유하고 설계" },
        { level: "프로", task: "공간·시간의 압축·분과 전문화", tech: "거리를 좁힌 철도·전신", expert: "분과별 심화 전문성" },
        { level: "아마추어", task: "인간 근력의 한계 극복", tech: "에너지를 생산하는 동력기관", expert: "정형화된 반복 노동" },
      ],
    ...
    },
    {
      ...
      ageColor: true,
      rows: [
        { level: "리더", task: "플랫폼·AI 거버넌스·가치 정렬 설계", tech: "AI·플랫폼 인프라·정렬 기술", expert: "기술이 추구할 목적과 생태계 규칙 설계" },
        { level: "프로", task: "디지털 시스템 구축·AI 감독·제도 설계", tech: "소프트웨어·AI 감독·데이터 분석", expert: "기술과 창의를 결합한 가치 생산·비판적 평가" },
        { level: "아마추어", task: "디지털 도구·AI 활용·데이터 생산", tech: "생성 AI·플랫폼·SNS", expert: "AI와 협업하며 삶의 목적을 설계" },
      ],
  ...
  return (
    <div style={{ pageBreakBefore: "always", height: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "10px 32px 8px", borderBottom: `2px solid ${C.navy}`, marginBottom: 8 }}>
        <div style={{ fontSize: 13, letterSpacing: 4, color: C.accent, fontWeight: 800, marginBottom: 4 }}>산업화는 근육을 대체했다. 디지털·AI는 두뇌를 대체한다 — 요구되는 인재상이 근본적으로 달라진다</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: C.navy, lineHeight: 1.35 }}>
          새 시대의 문제가 먼저 보여야, 그 문제를 풀 직업이 나오고, 그 직업에 맞는 정체성을 발견할 수 있다.
          산업화는 조직 관리자를 만들었지만, 디지털·AI는 AI와 함께 설계하는 혁신가를 요구한다.
        </div>
      </div>
      ...
      <div style={{ padding: "0 32px", flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {/* 2.1 개인과 전문성의 변화 */}
        <div>
          <div style={h3s}>2.1 개인과 전문성의 변화 — 시대마다 요구되는 인재상이 달라진다</div>
          <div style={{ fontSize: 12.5, lineHeight: 1.5, color: "#222", marginBottom: 5 }}>
            시대의 핵심 과제가 새로운 기술을 불러내고, 그 기술이 요구하는 전문성이 시대의 인재상을 빚어낸다.
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, marginBottom: 6 }}>
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                ...
                <th style={{ padding: "4px 6px", textAlign: "left", width: "9%" }}>핵심 원리</th>
                <th style={{ padding: "4px 6px", textAlign: "left", width: "17%" }}>핵심 과제</th>
                <th style={{ padding: "4px 6px", textAlign: "left", width: "20%" }}>핵심 기술</th>
                <th style={{ padding: "4px 6px", textAlign: "left", width: "18%" }}>핵심 전문성</th>
                <th style={{ padding: "4px 6px", textAlign: "left", width: "11%" }}>이상적 인재상</th>
                <th style={{ padding: "4px 6px", textAlign: "left" }}>필요한 자유</th>
              </tr>
            </thead>
            <tbody>
              {ageGroups.flatMap(({ age, principle, ageColor, rows, ideal, freedom }) =>
                rows.map((r, j) => ({
                  ...r, age, principle, ageColor, ideal, freedom,
                  isFirst: j === 0, rowCount: rows.length,
                }))
              ).map(({ age, principle, ageColor, isFirst, rowCount, task, tech, expert, ideal, freedom }, i) => (
                <tr key={i} style={{ background: ageColor ? "#f0f4ff" : i % 6 < 3 ? "#fff" : C.bg }}>
                  {isFirst && (
                    <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, fontWeight: 800, color: C.navy, lineHeight: 1.45, verticalAlign: "top", fontSize: 13 }}>
                      {age}
                    </td>
                  )}
                  {isFirst && (
                    <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, color: C.accent, fontWeight: 700, lineHeight: 1.5, verticalAlign: "top", fontSize: 11 }}>
                      {principle}
                    </td>
                  )}
                  ...
                  {isFirst && (
                    <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, color: "#222", fontWeight: 500, lineHeight: 1.45, verticalAlign: "top", fontSize: 11 }}>
                      {ideal}
                    </td>
                  )}
                  {isFirst && (
                    <td rowSpan={rowCount} style={{ padding: "4px 6px", borderBottom: `1px solid ${C.navy}`, borderTop: i === 0 ? "none" : `1px solid ${C.navy}`, color: "#222", lineHeight: 1.45, verticalAlign: "top", fontWeight: 500, fontSize: 11 }}>
                      {freedom}
                    </td>
                  )}
                </tr>
