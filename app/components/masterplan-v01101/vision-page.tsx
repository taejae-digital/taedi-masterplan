...
function H3({ no, title }: { no: string; title: string }) {
  return (
    <div style={{ display: "flex", alignItems: "baseline", gap: 11, margin: "0 0 12px", paddingBottom: 8, borderBottom: `2px solid ${C.ink}` }}>
      <span style={{ fontSize: 20, fontWeight: 900, color: C.accent, letterSpacing: -0.5 }}>{no}</span>
      <span style={{ fontSize: 18.5, fontWeight: 800, color: C.ink, letterSpacing: -0.4 }}>{title}</span>
    </div>
  );
}
...
export function VisionPage() {
  return (
    <div style={{ minHeight: "281mm", display: "flex", flexDirection: "column", background: "#fff" }}>
      ...
      {/* 인과 흐름 4스텝 */}
      <div style={{ display: "flex", alignItems: "stretch", padding: "16px 48px 16px", gap: 0 }}>
        {flow.map((f, i) => (
          <div key={f.k} style={{ display: "flex", flex: 1, alignItems: "stretch" }}>
            <div style={{ flex: 1, background: C.cardBg, border: `1px solid ${C.cardBorder}`, borderTop: `4px solid ${f.c}`, borderRadius: 10, padding: "13px 15px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 7, marginBottom: 6 }}>
                <span style={{ fontSize: 15, fontWeight: 900, color: f.c, letterSpacing: -0.5 }}>{f.k}</span>
                <span style={{ fontSize: 11, fontWeight: 800, color: C.muted, letterSpacing: 1 }}>{f.label}</span>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 11.3, color: C.body, lineHeight: 1.4, marginTop: "auto" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "12px 1fr", gap: 5 }}><span style={{ color: f.c, fontWeight: 900 }}>—</span><span style={{ fontWeight: 600, color: C.ink }}>{f.digital}</span></div>
              </div>
            </div>
            {i < flow.length - 1 && <div style={{ display: "flex", alignItems: "center", padding: "0 10px", fontSize: 22, color: C.faint, fontWeight: 300 }}>→</div>}
          </div>
        ))}
      </div>
