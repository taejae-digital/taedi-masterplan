...
const contractRows = [
  { label: "형성", text: "정체성을 발견하고 훈련할 기회" },
  { label: "발현", text: "AI·데이터·시장에 접근할 조건" },
  { label: "조정", text: "플랫폼 규칙을 설명·거부·수정할 권리" },
];

const contractConditions = ["AI·데이터·컴퓨팅 접근", "학습·건강 기반", "시장·도시 연결", "설명·거부·조정 UI"];
  ...
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "270mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "4px 38px 3px", display: "flex", flexDirection: "column", flex: 1, gap: 4 }}> 
        ...
        <div style={{ flex: 1 }}>
          <div style={h3s}>3.3 새로운 사회계약 — 개인의 정체성이 발휘될 기본 여건</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ padding: "10px 14px", background: C.navy, color: "#fff", textAlign: "center" }}>
              <div style={{ fontSize: 20.5, lineHeight: 1.14, fontWeight: 950 }}>
                새로운 사회계약은<br />개인의 정체성이 발휘될 기본 여건을 보장한다.
              </div>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {contractRows.map((r, i) => (
                <div key={r.label} style={{ border: `1.5px solid ${i === 2 ? C.accent : C.navy}`, background: i === 2 ? "#fff4ec" : "#fff" }}>
                  <div style={{ background: i === 2 ? C.accent : C.navy, color: "#fff", padding: "6px 9px", fontSize: 12.3, fontWeight: 900 }}>{r.label}</div>
                  <div style={{ padding: "9px 10px", fontSize: 14.2, lineHeight: 1.16, color: C.navy, fontWeight: 850 }}>{r.text}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 7 }}>
              {contractConditions.map((r) => (
                <div key={r} style={{ padding: "8px 8px", background: "#eef3ff", borderTop: `4px solid ${C.navy}`, color: C.navy, fontSize: 12.2, lineHeight: 1.12, fontWeight: 800, textAlign: "center" }}>{r}</div>
              ))}
            </div>
          </div>
        </div>
