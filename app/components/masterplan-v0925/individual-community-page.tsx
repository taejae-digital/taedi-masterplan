...
export function IndividualCommunityPage() {
  ...
  // 컬럼 구조:
  // [왼쪽 블록] 시대 | 핵심 원리 | 추구하는 자유  ← 오른쪽 굵은 테두리
  // [오른쪽 블록] 핵심 과제 | 핵심 기술 | 핵심 전문성 | 이상적 인재상
  const ageGroups = [
    {
      age: "농업",
      principle: "정착 + 위계",
      ...
      ageColor: false,
      rows: [
        { task: "위계 질서의 정당화", tech: "위계를 신성화한 종교", expert: "의례·통치의 권위" },
        { task: "세대 간 지식의 전승", tech: "기록을 가능케 한 문자", expert: "경험의 축적과 전승" },
        { task: "식량 생산의 안정화", tech: "물을 다스리는 관개술", expert: "체화된 생산의 기예" },
      ],
    ...
    },
    {
      ...
      ageColor: false,
      rows: [
        { task: "대규모 조직의 운영·자본 조달", tech: "분업을 제도화한 관료제·조립라인", expert: "생산 시스템을 소유하고 설계" },
        { task: "공간·시간의 압축·분과 전문화", tech: "거리를 좁힌 철도·전신", expert: "분과별 심화 전문성" },
        { task: "인간 근력의 한계 극복", tech: "에너지를 생산하는 동력기관", expert: "정형화된 반복 노동" },
      ],
    ...
    },
    {
      ...
      ageColor: true,
      rows: [
        { task: "플랫폼·AI 거버넌스·가치 정렬 설계", tech: "AI·플랫폼 인프라·정렬 기술", expert: "기술이 추구할 목적과 생태계 규칙 설계" },
        { task: "디지털 시스템 구축·AI 감독·제도 설계", tech: "소프트웨어·AI 감독·데이터 분석", expert: "기술과 창의를 결합한 가치 생산·비판적 평가" },
        { task: "디지털 도구·AI 활용·데이터 생산", tech: "생성 AI·플랫폼·SNS", expert: "AI와 협업하며 삶의 목적을 설계" },
      ],
      ideal: "T자형 인간 / 조율자(orchestrator)",
    },
  ];

  // 구분선 스타일 (추구하는 자유 열 오른쪽 굵은 테두리)
  const dividerBorder = `3px solid ${C.navy}`;
        ...
        <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5, marginBottom: 6 }}>
            ...
            <tbody>
              ...
              ).map(({ age, principle, freedom, ageColor, isFirst, rowCount, task, tech, expert, ideal }, i) => {
