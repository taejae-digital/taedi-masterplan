...
const limitRows = [
  {
    ...
    strategy: "한국은 문화 신뢰, 반도체 공급망, 빠른 AI 활용 시민을 묶어 미·중이 함께 쓸 수밖에 없는 검증 시장을 만든다.",
    strategyBullets: [
      "미국에는 반도체·AI 인프라 공급망, 민주주의형 검증 시장, 빠른 시민 AI 활용 데이터를 제공한다.",
      "중국에는 문화적 신뢰와 사회 안정형 AI 검증 참여 통로, 고령화·도시 문제 실증 기회를 제공한다.",
      "그 대가로 데이터 주권·공공 감사 로그·고위험 AI 검증을 공동 운영 규칙으로 묶는다.",
    ],
  },
  {
    title: "한국의 위치",
    problem: "패권국가에 의존하는 산업구조로는 새 사회계약을 요구할 협상 카드를 만들기 어렵다.",
    problemBullets: [
      "디지털 인프라 접근권이 외부 플랫폼·칩·클라우드 의존에 갇히면 권리가 아니라 사용 허가가 된다.",
      "직접 거버넌스 참여권은 산업·공급망·시장 접근을 걸고 협상할 카드가 없으면 선언에 머문다.",
      "정체성 실현권은 한국의 문화·교육·도시 실증이 산업 주도권과 연결될 때만 세계에 제안할 모델이 된다.",
    ],
    strategy: "한국은 산업 접근과 도시 실증을 제공하고 새 사회계약의 조건을 받아내는 협상 국가가 된다.",
    strategyBullets: [
  ...
  {
    title: "거대도시의 비효율성",
    problem: "거대도시는 주거·일·교육·돌봄이 흩어져 AI가 개인의 성장 경로로 작동하기 어렵다.",
    problemBullets: [
      "집은 비싸고, 일터는 멀고, 학교·병원·돌봄·행정은 서로 따로 움직인다.",
      "디지털 인프라 접근권이 장비 보급에 머물고 학습·건강·일자리·참여 기록으로 이어지지 않는다.",
      "직접 거버넌스 참여권은 생활 문제 해결과 연결되지 않고, 정체성 실현권은 개인 성장 기록으로 남지 않는다.",
    ],
    strategy: "강소도시는 주거·일·교육·돌봄을 연결해 AI가 개인 성장에 쓰이는 생활권을 만든다.",
    strategyBullets: [
      "도시는 기업·대학·연구기관에 생활권 실증 시장과 공공조달 기회를 제공한다.",
  ...
  },
  {
    ...
    strategy: "가정은 생활 데이터를 제공하되 성장 서비스와 데이터 권리를 돌려받는 최소 사회계약 단위가 된다.",
    strategyBullets: [
...
function BulletList({ items, fontSize = 9.55 }: { items: string[]; fontSize?: number }) {
  return (
    <ul style={{ margin: 0, paddingLeft: 14, display: "flex", flexDirection: "column", gap: 2.2 }}>
      {items.map((item) => (
        <li key={item} style={{ margin: 0, paddingLeft: 1, fontSize, lineHeight: 1.18 }}>{item}</li>
      ))}
    </ul>
  );
}
...
export function GovernanceExpansionPage() {
  return (
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 36px 14px", display: "flex", flexDirection: "column", flex: 1, gap: 8 }}>
        <section>
          <div style={h3s}>4.1 새로운 사회계약에 이르기 위한 전략</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
            {limitRows.map((row, i) => (
              <div key={row.title} style={{ minHeight: 194, display: "grid", gridTemplateColumns: "98px 1fr", border: `1px solid #d7d0c7`, background: i % 2 ? C.bg : "#fff", boxShadow: "0 1px 0 rgba(0,0,0,0.03)" }}>
