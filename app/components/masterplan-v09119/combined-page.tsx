...
const econRows: Row[] = [
  {
    ...
    next: "작고 구체적인 필요를 해결하는 사람이 가치를 만든다 — 정체성 실현인이 새 생산 주체",
  },
  {
    dim: "분배",
    prev: "임금 + 노동소득세 + 복지",
...
const classRows: ClassRow[] = [
  ...
  },
  {
    ...
    digital: {
      ...
      distribution: "작고 구체적인 필요 해결로 차별화",
...
const polRows: Row[] = [
  {
    ...
    why: "국가 입법권 침식. 욕망을 최대한 실현할 자유(인권·참정권·사유재산권) 위에 짠 산업시대 입법 틀로 정체성 위협 못 막음",
      ...
      <div style={{ padding: "6px 32px 8px", display: "flex", flexDirection: "column", flex: 1, gap: 7 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11.2, color: "#222", lineHeight: 1.35, padding: "4px 8px", background: "#f0f4ff", borderLeft: `3px solid ${C.navy}`, marginBottom: 6 }}>
            <strong style={{ color: C.navy }}>산업시대 경제는 노동가치설 위에 섰다.</strong> 디지털시대는 <strong>AI가 능력을 확장</strong>한다 — 디자인·코딩·회계·번역을 비전문가가 직접 한다. 가치는 오래 일한 시간이 아니라 <strong>작고 구체적인 필요를 발견하고 여러 기능을 조합해 해결하는 능력</strong>에서 나온다. 그래서 같은 4계층(리더·프로·아마추어·소외)도 산업시대와 디지털시대에 <strong>역할과 비중이 달라진다</strong>.
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "17%" }} />
        ...
        {/* 3.3 새로운 사회계약 — 정체성 실현을 뒷받침하는 권리 구조 */}
        <div style={{ flex: 1 }}>
          <div style={h3s}>3.3 새로운 사회계약 — 정체성 실현을 뒷받침하는 권리 구조</div>
          <div style={{ fontSize: 11.2, color: "#222", lineHeight: 1.35, padding: "4px 8px", background: "#f0f4ff", borderLeft: `3px solid ${C.navy}`, marginBottom: 6 }}>
            <strong style={{ color: C.navy }}>정체성 실현권</strong>은 디지털시대 인간이 자기 삶의 미션을 발견·훈련·실현할 권리다. 일종의 천부인권처럼 권리 구조의 중심에 놓이고, 그 아래에서 <strong>디지털 인프라 접근권</strong>은 능력 조건을, <strong>직접 거버넌스 참여권</strong>은 공동체 규칙으로 연결되는 통로를 맡는다.
          </div>
            ...
            <tbody>
              {[
                ...
                },
                {
                  ...
                  contract: "제도 연결 — 정체성 실현을 공동체 규칙과 제도 변화로 연결",
                },
              ].map((r, i) => (
