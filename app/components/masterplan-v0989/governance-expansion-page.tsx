...
const industryAxisRows = [
  ["AI·반도체·연산 축", "AI 모델·반도체·데이터센터·전력망 경쟁에서 한반도는 제조·메모리·전력·데이터 운영의 안정 거점이 된다.", "서해 AI·첨단제조 벨트: 수도권–충청–새만금–호남 서해안", "대학은 AI·반도체 커리큘럼, 기업은 실습라인, 도시는 데이터센터·전력·주거를 묶어 인재 정착을 설계한다.", "싱크탱크=전략맵, 대학=인재, 기업=공급망, 정부=전력·데이터 법제"],
  ["배터리·소재·첨단제조 축", "세계 공급망이 소재·부품·장비와 제조 신뢰성을 요구할수록 한반도는 배터리·디스플레이·정밀제조의 대체불가 생산기지가 된다.", "서해–동남 첨단제조 연결축: 충청–전북–울산–창원–포항", "도시는 공장만 유치하지 않고 대학 실험실, 테스트베드, 주거·돌봄, 직무전환 학교를 한 묶음으로 운영한다.", "기업=제조 실행, 대학=소재 인재, 정부=공공조달, 정당=지역산업 권한 구조"],
  ["해양물류·로봇·수소 축", "세계 교역과 에너지 전환이 항만·선박·물류 자동화를 요구할 때 한반도 동남권은 해양 제조와 스마트항만의 실험장이 된다.", "동남 해양제조·물류 벨트: 부산–울산–창원–거제–포항", "항만도시는 조선·로봇·물류 데이터를 열고, 대학·기업·노동자·도시정부가 안전 규칙과 자동화 전환을 공동검증한다.", "군대=해양안보·재난, 기업=항만 자동화, 대학=로봇·해양공학, 정부=항만 규칙"],
  ["에너지·기후·바이오 축", "기후위기와 고령화가 에너지 안정성·건강 데이터·돌봄 산업을 키울 때 한반도는 에너지·바이오·기후 적응의 생활 실험장이 된다.", "동해 에너지·기후·바이오 벨트: 강원–동해안–경북 내륙", "도시는 병원·대학·에너지 시설·돌봄기관을 연결해 건강·환경 데이터가 개인 삶의 질 개선으로 환류되게 한다.", "대학=바이오·기후 연구, 정부=환경 데이터, 미디어=기후 리터러시, 기업=헬스케어"],
  ["문화·교육·관광 플랫폼 축", "세계 문화 플랫폼 경쟁에서 한반도는 콘텐츠·교육·관광·지역 커뮤니티를 묶어 정체성 발견 산업을 수출하는 거점이 된다.", "남부 문화·교육·관광 벨트와 북방 연결축: 광주–전주–남해안–제주–수도권 북부", "도시는 축제·학교·미디어·관광·창작공간을 연결하고, 개인이 콘텐츠 제작·학습·지역 프로젝트로 자기 정체성을 실험하게 한다.", "미디어=콘텐츠 확산, 대학=교육모듈, 기업=플랫폼, 싱크탱크=북방·문화전략"],
];
    ...
    <div style={{ pageBreakBefore: "always", minHeight: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 30px 7px", display: "flex", flexDirection: "column", flex: 1, gap: 5 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10.4, color: "#444", lineHeight: 1.25, marginBottom: 3, fontWeight: 650 }}>새 권리는 추상 원칙에 머물지 않는다. 가정의 생활 AI, 도시의 산업 테마, 국가의 한반도 산업축, 세계의 규칙 경쟁 속에서 실행되어야 한다.</div>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "10%" }} />
              <col style={{ width: "22%" }} />
        ...
        <div>
          <div style={{ ...h3s, fontSize: 15.2, marginBottom: 2 }}>4.3-1 세계 산업축 → 한반도 역할 → 도시 시산학 구현</div>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "14%" }} />
              <col style={{ width: "22%" }} />
            ...
            <thead>
              <tr style={{ background: C.navy, color: "#fff" }}>
                ...
                <th style={th}>한반도 산업축</th>
                <th style={th}>도시 시산학 구현</th>
                <th style={th}>7대분야 실행</th>
              </tr>
            </thead>
            <tbody>
              {industryAxisRows.map(([axis, role, koreaAxis, city, fields], i) => (
                <tr key={axis} style={{ background: i % 2 ? C.bg : "#fff" }}>
                  <td style={{ ...td, color: C.navy, fontWeight: 900, fontSize: 8.45 }}>{axis}</td>
                  <td style={td}>{role}</td>
                  <td style={td}>{koreaAxis}</td>
                  <td style={{ ...td, color: C.navy, fontWeight: 650 }}>{city}</td>
                  <td style={{ ...td, color: C.accent, fontWeight: 800, fontSize: 8.05, lineHeight: 1.08 }}>{fields}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const td: React.CSSProperties = { padding: "2.6px 4px", borderBottom: "1px solid #eee", fontSize: 8.2, lineHeight: 1.08, color: "#333", verticalAlign: "top" };
