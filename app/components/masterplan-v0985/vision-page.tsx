...
const flow = [
  ...
  },
  {
    ...
    d: "AI가 직무 기능을 개인에게 이전하면 한 사람이 여러 직업의 일을 조합해 세분화된 니즈를 겨냥할 수 있다. 그래서 직업명보다 ‘나는 누구의 어떤 필요를 풀 것인가’가 핵심 질문이 된다.",
    bg: "#fff8f4",
    c: C.red,
  },
  {
    k: "1.3 새 사회계약",
    t: "세분화된 니즈를 새 가치로 연결한다",
    d: "가치의 기준이 노동시간에서 세분화된 니즈를 발견하고 충족하는 능력으로 이동하므로, 디지털 인프라 접근·직접 참여·정체성 실현을 권리로 보장한다.",
    bg: "#f0f5ff",
    c: "#2563eb",
  },
  {
    ...
    <div style={{ height: "281mm", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 32px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, flex: 1 }}>
        <div>
          ...
          <p style={ps}>
            디지털시대에는 이 틀이 약해진다. AI가 분석·창작·기획·실행 능력을 개인에게 이전하면, 한 사람이 여러 직업의 기능을 동시에 조합하고 과거 조직 단위의 산출에 도달할 수 있다. 동시에 디지털 시장은 대중 평균을 겨냥한 표준 상품보다, 작고 구체적인 니치마켓의 필요를 더 많이 드러낸다. 이때 직업명은 개인이 어떤 세분화된 니즈를 이해하고 충족할 수 있는지 설명하지 못한다. 중요한 것은 어떤 직업에 속했는지가 아니라 <strong>누구의 어떤 필요를 발견하고, 어떤 조합의 능력으로 충족시킬 것인가</strong>다. 그래서 개인에게 남는 질문은 “무슨 직업을 가질 것인가”가 아니라 <strong>“어떤 정체성으로 누구의 필요를 풀 것인가”</strong>다.
          </p>
        ...
        <div>
          <p style={ps}>
            디지털시대에는 이 전제가 흔들린다. 산업시대에는 같은 시간, 같은 공정, 같은 직무를 수행하는 노동이 가치 측정의 중심이었다. 그러나 AI가 정형화된 인지노동을 대체하면, 단순히 오래 일하는 것보다 세분화된 니즈를 발견하고, 여러 기능을 조합해, 특정한 사람과 상황에 맞는 결과를 만들어내는지가 더 중요해진다. 여기서 정체성은 취향이나 자기표현이 아니라 <strong>어떤 필요를 중요하게 보고 어떤 방식으로 충족시킬지를 결정하는 기준</strong>이다. 그래서 가치는 노동시간이 아니라 세분화된 니즈를 충족시키는 정체성 기반의 기여에서 나온다.
          </p>
          <p style={ps}>
