...
export function IndividualCommunityPage() {
  ...
  const lifeStages = [
    {
      ...
      indTags: ["가정", "국가"],
      digTags: ["가정", "국가"],
      industrialPerson: "가정 환경",
      digitalPerson: "부모·생체·유전 데이터로 정체성 단서를 조기 발견",
    ...
    },
    {
      ...
      indTags: ["가정", "도시"],
      digTags: ["가정", "이웃"],
      industrialPerson: "조기교육·성적 격차",
      digitalPerson: "여러 정체성 단서를 체험으로 확인·검증",
    ...
    },
    {
      ...
      indTags: ["도시"],
      digTags: ["마을", "도시"],
      industrialPerson: "대학입시 성적",
      digitalPerson: "사회 기여가 큰 정체성을 선정해 집중 훈련·개발",
    ...
    },
    {
      ...
      indTags: ["도시"],
      digTags: ["도시"],
      industrialPerson: "학벌·취업시장 진입",
      digitalPerson: "정체성 실현 — 창업·전문직·창작으로 발휘",
    ...
    },
    {
      ...
      indTags: ["도시", "국가"],
      digTags: ["가정", "도시", "국가"],
      industrialPerson: "조직 내 직위·고용 안정성",
      digitalPerson: "정체성 고도화 — 경험·데이터로 영향력 확장",
    ...
    },
    {
      ...
      indTags: ["국가"],
      digTags: ["가정", "이웃", "마을"],
      industrialPerson: "은퇴·자산·건강 상태",
      digitalPerson: "정체성 전수·연결 — 멘토링·자문·세대 연결",
  ...
  type PersonValue = string | { lead: string; pro: string; ama: string; margin: string };
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
        {CLASS_ROWS.map((r) => (
          <div key={r.key} style={{ display: "flex", alignItems: "flex-start", gap: 5 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 650, color: C.muted, lineHeight: 1.26 }}>{value[r.key]}</span>
          </div>
        ))}
      </div>
    ), { borderTop: true, bg });
    ...
    <div style={{ pageBreakBefore: "always", height: "281mm", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "10px 48px 0", flex: 1, display: "flex", flexDirection: "column", gap: 8, justifyContent: "space-between" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "86px repeat(6, 1fr)", gap: 0, border: `1px solid ${C.cardBorder}`, borderRadius: 4, overflow: "hidden" }}>
          ...
          {rowLabel("산업화 개인", "4계층 구분 기준", "#6b7280")}
          ...
          {rowLabel("디지털 개인", "4계층 구분 기준", "#6b7280")}
