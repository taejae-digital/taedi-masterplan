...
const flow = [
  ...
  },
  {
    ...
    t: "개인의 강화는 공동체를 기능 결합에서 기여 연결로 바꾼다",
    industrial: "산업화 공동체: 분업화된 개인의 기능을 결합해 효율을 생산",
    digital: "디지털 공동체: 방향을 공유한 개인들의 서로 다른 기여를 연결해 시너지를 조직",
    c: "#555555", soft: "#ffffff",
  },
  {
  ...
  return (
    <div style={{ height: "281mm", overflow: "hidden", display: "flex", flexDirection: "column", background: "#fff" }}>
      ...
      <div style={{ padding: "6px 48px 0", display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: 42, rowGap: 18, flex: 1 }}>
        ...
        <div>
          <H3 no="1.2" title="개인의 강화와 공동체의 변화 — 기능 결합에서 기여 연결로 바뀐다" />
          <p style={ps}>
            개인이 강화되면 공동체의 역할도 바뀐다. 산업화 시대의 핵심 질문은 “어디에 취업할 것인가”였다. 개인은 기업과 공동체가 정한 목적 안에서 자신의 기능을 제공했고, 공동체는 분업화된 개인의 기능을 결합해 효율을 생산했다. 그러나 AI가 평균적 수행을 빠르게 대체하면 단순히 주어진 일을 잘하는 능력의 가치는 급격히 줄어든다.
          </p>
          <p style={ps}>
            앞으로 중요한 것은 어떤 기능을 수행하느냐가 아니라 <strong style={em}>무엇을 중요하게 보고, 누구의 어떤 필요를 풀 것인가</strong>다. 산업화 시대에 리더와 사업가가 하던 질문을, 디지털 시대에는 강화된 개인들이 직접 하게 된다. 그러려면 개인은 표준 직무·경로에 묶이지 않고 자기 문제의식과 고유한 기여를 실험할 <strong style={em}>더 큰 자유</strong>를 가져야 한다. 정체성은 취향이나 자기표현이 아니라, 개인화된 생산에서 고유한 기여를 만드는 기준이다. 따라서 공동체도 표준화된 기능을 결합하는 장치에서, 방향을 공유한 개인들의 서로 다른 기여를 연결하는 시너지 조직으로 바뀐다. 요차이 벤클러(하버드대 교수)는 <em style={{ fontStyle: "normal", color: C.muted, fontWeight: 600 }}>『네트워크의 부』(2006)에서 강화된 개인들이 위계 없이 모여 가치를 만드는 동료생산(peer production)이 새 생산양식이 된다고 본다.</em>
          </p>
        </div>
