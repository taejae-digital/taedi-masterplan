...
para(
    ...
    "그래서 데이터가 닿지 않는 아이를 먼저 찾아가 발달을 지원하는 일은, 아이의 삶에 가장 가까운 도시와 마을이 맡는 것이 효과적이다. "
    "국가는 직접 제공자가 아니라, 어느 도시에 태어나도 기초 돌봄에서 밀려나지 않도록 도시 간 격차를 메우는 최소 보장선을 까는 데 그친다.",)
...
rich([("첫째, 위임의 선택권을 개인에게 돌려주어야 한다. ", True),
      ("국가 하나가 모든 생애단계의 필요를 독점 관리하는 구조에서, 개인이 필요에 따라 가정·이웃·마을·도시에 "
       ...
       "위임의 대상을 스스로 선택하고 바꿀 수 있는 구조가 함께 필요하다.", False)])
