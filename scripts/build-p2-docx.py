...
def para(text="", size=10.5, bold=False, italic=False, align=None,
         space_before=0, space_after=6, line=1.6, font=BODY_FONT, indent_first="auto"):
    p = doc.add_paragraph()
...
para(
    ...
    "개인이 먼저 있고, 생애 단계마다 필요가 달라지면 그 필요를 감당하기 위해 공동체가 구성된다. "
    "위임이 모여 공동체가 서며, 그 공동체가 더 이상 필요하지 않을 때 개인은 위임한 권리를 회수할 수 있어야 한다. "
    "이 연구는 위임과 회수의 계약 논리를 토대로 농업·산업화·디지털 세 시대의 노동 원리, 생애주기별 필요, "
    "그 필요에 응답하는 공동체의 형성과 역할을 정합적으로 연결한다. "
    "산업화 시대에는 표준 교육과 안정된 고용이라는 당대의 필요에 맞추어 권리가 국가로 집중되었다. "
    "이 집중은 당대의 필요에는 부합했으나 정체성 발휘를 요구하는 디지털 시대의 필요에는 맞지 않는다. "
    "디지털 전환은 집중된 권리를 가정·이웃·마을·도시·국가·세계의 여러 층위로 다시 분산하는 과정으로 해석된다. "
    "논의는 홉스·로크·루소의 사회계약론을 토대로 삼고, 에릭슨의 생애 발달 이론, 브론펜브레너의 생태체계 이론, "
    "오스트롬의 다중심 거버넌스, 센·누스바움의 역량 접근, 폴라니의 탈배태·재배태 개념, "
    "마이클 영의 능력주의 비판, 에스핑안데르센의 사회투자론, 헥만의 생애 초기 투자 연구로 보완된다.",
    space_after=8)
rich([("주제어  ", True),
...
para(
    ...
    "부모가 자녀를 위해 마을 돌봄 조직에 시간과 결정권의 일부를 맡기는 것이 위임이며, "
    "이주하거나 그 조직이 역할을 다하지 못한다고 판단하면 위임을 회수할 수 있어야 한다. "
    "이 관점은 공동체를 개인에 앞서 존재하는 유기체가 아니라 개인의 선택과 위임으로 구성·유지·해소되는 "
    "계약적 실체로 본다는 점에서 기존 공동체론과 구별된다.")
para(
    "토머스 홉스(Thomas Hobbes, 1588-1679)는 『리바이어던』(1651)에서 만인의 만인에 대한 투쟁을 "
    "벗어나기 위해 개인이 권리를 통치자에게 양도하는 계약을 제시했으나, 그 모형에서 위임은 사실상 회수 불가능하다. "
    "존 로크(John Locke, 1632-1704)는 『통치론』(1689)에서 통치자에게 권력을 맡기는 것은 신탁(trust)이며 "
    "그 신탁이 깨지면 권리를 회수할 수 있다고 보았다. 로크의 신탁과 저항권은 이 연구가 전제하는 위임 회수 가능성의 직접적 근거다. "
    ...
    "세 논의를 종합하면, 공동체는 개인의 권리 위임으로 성립하고(홉스), 그 위임은 회수 가능해야 하며(로크), "
    "권리는 한 곳에 집중되어서는 안 된다(루소).")
para(
    "이 토대 위에서 세 가지 물음을 검토한다. 시대별 노동 원리는 개인의 필요와 공동체의 형태를 어떻게 규정했는가. "
    "생애주기 단계별로 달라지는 필요에 어떤 공동체가 응답했는가. 디지털 전환은 왜 권리 재분산의 관점에서 분석되어야 하는가.")
...
para(
    ...
    "공동체의 규모와 권한은 위임된 권리의 양에 따라 결정된다. 개인은 혼자 감당할 수 없는 필요가 생기면 "
    "그 필요를 충족할 공동체에 권리를 위임하고, 필요가 변하면 위임의 대상과 규모도 달라진다. "
    "청년기에는 도시와 직장에 많은 권리를 위임하고, 중장년기에는 가정과 이웃으로 위임의 무게중심이 이동하며, "
    "노년기에는 돌봄과 존엄을 위한 공동체에 대한 의존이 커진다.")
para(
...
para(
    "로크의 신탁 이론은 위임이 조건부임을 함의한다. 국가가 권리를 올바르게 사용하리라는 신탁이 깨지면 권리는 개인에게 회복되어야 한다. "
    "그러나 산업화 시대에는 회수 경로가 제한되었다. 국가가 교육·고용·복지·의료를 중앙에서 관리했고, "
    "개인은 국가 교육을 거쳐 인증된 기업에 취업하고 노년에 국가 연금에 의존하는 단선 경로에 편입되었다. "
    "이 경로 바깥에 서는 것은 사회적 배제를 의미했다. 장애인이나 소수 언어 사용자처럼 표준 경로가 맞지 않는 사람은 제도 안에서 드러나지 않았다.")
para(
    "이 과정에서 이웃과 마을은 공동 돌봄 기능을 상실했고 가정의 역할은 축소되었으며 국가의 기능은 확대되었다. "
    "권리가 집중된 공동체는 다양한 개인의 필요에 개별적으로 대응하기 어렵다. 표준화는 평균 바깥의 개인을 배제하고 다양성을 제약한다.")
para(
    "칼 폴라니(Karl Polanyi, 1886-1964)는 『거대한 전환』(1944)에서 산업화 시대 시장 경제가 노동·토지·화폐를 "
    "사회적 관계에서 분리하여 상품으로 만들었다고 분석하고, 이를 탈배태(disembedding)로 명명했다. "
    "산업화 시대는 이웃·마을·가정에 분산되어 있던 권리 위임을 국가로 집중한 시대였다. "
    "디지털 시대의 과제는 그 집중을 여러 층위로 되돌리는 재배태(re-embedding)다.")

h2("2.3", "다층 자치와 생태체계")
para(
    "엘리너 오스트롬(Elinor Ostrom, 1933-2012)은 사람들이 중앙 정부나 사유화 없이도 자치 규칙을 만들어 "
    "공유 자원을 지속 가능하게 관리할 수 있음을 실증했다(『공유의 비극을 넘어』, 1990). "
    "그는 이를 다중심 거버넌스(polycentric governance), 곧 여러 층위의 공동체가 각자의 영역에서 문제를 분담하는 구조로 설명했다. "
    "권리는 가정·이웃·마을·도시·국가·세계가 층위에 따라 분담해야 하며, 한 층위가 권리를 독점하면 나머지 층위는 약화되고 개인의 선택지는 줄어든다.")
para(
    "유리 브론펜브레너(Urie Bronfenbrenner, 1917-2005)는 개인의 발달을 중첩된 생태체계 안에서 분석했다"
    "(『인간 발달의 생태학』, 1979). 가정(미시체계), 이웃과 학교(중간체계), 부모의 직장(외체계), 사회의 가치 규범(거시체계)이 "
    "상호작용한다. 그의 핵심 명제는 개인에게 가장 큰 영향을 미치는 것이 가까운 공동체와의 직접적 상호작용, 곧 근접 과정(proximal process)이라는 것이다. "
    "산업화 시대는 근접 과정이 일어나는 가정·이웃·마을을 약화시키고 권리를 거시체계인 국가로 집중함으로써 개인 발달의 토대를 약화시켰다.")
...
para(
    ...
    "한국의 품앗이는 비공식적이지만 정교한 권리 위임과 회수의 사례다. 생애주기도 협동의 논리를 따라, "
    "아동기는 공동 노동의 준비기, 청년기는 혼인과 토지 상속으로 새 가정을 구성하는 시기, 중장년은 마을 자치의 중심, 노년은 봉양의 대상이었다.")
...
para(
    "이 전환은 권리 위임 방식도 바꾼다. 국가에 집중되었던 권리가 여러 공동체로 다시 분산될 조건이 형성된다. "
    "가정은 정체성의 단서를 가장 먼저 읽는 공간으로서 중요성을 회복하고, 이웃과 마을은 정체성을 실험하고 훈련하는 관계망이 된다. "
    "도시는 정체성을 실제 성과로 발휘하는 장이 되며, 국가는 모든 것을 관리하는 기관에서 여러 공동체가 작동하도록 최소 조건을 보장하는 조율자로 전환된다. "
    "세계는 처음으로 개인의 발휘 영역으로 열린다. 국경을 넘는 협업이 가능해지면서 국내 노동시장에서 자리를 얻지 못한 사람도 세계 무대에서 성과를 낼 경로를 갖는다.")
...
para(
    ...
    "제임스 헥만(James Heckman, 1944- )은 생애 초기 양육·교육 환경에 대한 투자가 이후 어느 시기보다 높은 복리 효과를 낳으며, "
    "성인기의 범죄 예방·소득 향상·복지 지출 감소로 수 배의 사회적 수익을 낳음을 실증했다. "
...
para(
    ...
    "가정은 관심을 지지하고, 이웃은 체험 기회를 제공하며, 마을은 공동 발견 프로그램을 운영한다. "
    "가정이나 국가가 교육을 독점하는 구조에서 가정·이웃·마을이 역할을 분담하는 분산형 위임 구조로 이행하는 것이 이 단계의 핵심이다. "
    "브론펜브레너가 중간체계(mesosystem)로 규정한 가정·이웃·마을의 상호 강화 구조가 이 단계에서 작동해야 한다. "
    "체험이 풍부한 아이와 그렇지 못한 아이의 격차는 이미 이 단계에서 발생하므로 국가는 기회의 바닥을 보장하여 분산 구조를 뒷받침해야 한다.")

h2("4.3", "청소년기")
para(
    "청소년기는 정체성의 위기가 가장 첨예한 시기다. 에릭 에릭슨(Erik H. Erikson, 1902-1994)은 생애 발달을 여덟 단계로 나누어 "
    "각 단계의 심리사회적 과제를 제시했다(『아동기와 사회』, 1950). 청소년기의 과제는 정체성 대 역할 혼미(identity vs. role confusion)다. "
    "농업 시대에는 가업 기술 습득이, 산업화 시대에는 입시 경쟁과 진학이 이 필요를 충족했다. 성적이 정체성을 규정했고 도시의 입시 교육과 국가의 대입 제도가 "
    "청소년의 권리 위임을 독점했다. 디지털 시대에는 마을이 멘토와 훈련 경로를 제공하고 도시가 심화 경로와 재진입 통로를 마련하여 "
    "각자의 강점이 점수 서열 바깥에서 길러질 조건을 형성한다.")

h2("4.4", "청년기")
para(
    "청년기는 훈련으로 다듬어진 강점을 실제 성과로 발휘하는 시기다. 농업 시대에는 토지와 혼인을 통한 새 가정 구성이, 산업화 시대에는 안정된 직장 취업과 정착이 목표였고 "
    "두 경우 모두 필요는 외부가 규정했으며 국가와 도시가 경로를 통제했다. 디지털 시대 청년의 필요는 정체성을 실제 성과로 발휘할 영역이다. "
    "인공지능과 플랫폼이 진입 비용을 낮추면서 개인은 대기업에 고용되지 않고도 가치를 생산할 수 있다. "
    "산업화 시대 청년이 노동력과 결정권을 기업과 국가에 대규모로 위임하는 것이 거의 유일한 선택지였다면, "
    "디지털 시대에는 도시에 발휘의 기반을 위임하고 세계와 선택적으로 협업하며 재편할 수 있는 유연한 위임 구조가 가능하다. "
    "예스타 에스핑안데르센(Gøsta Esping-Andersen, 1947- )은 지식경제 시대에는 복지를 소득 보전에 한정하지 말고 "
    "교육·보육·재훈련 같은 사람에 대한 사전 투자, 곧 사회투자(social investment)로 전환해야 한다고 강조했다.")

h2("4.5", "중장년기")
para(
    "중장년기는 디지털 전환의 혜택과 위험이 가장 불균등하게 갈리는 시기다. 산업화 시대의 목표는 고용 유지와 가족 부양이었고 권리 위임은 직장과 국가를 향했다. "
    "안정된 고용이 유지되는 한 개인은 노동 시간과 역량을 기업에, 연금 권리를 국가에 위임하는 계약을 따랐다. "
    "디지털 전환은 자동화와 인공지능이 기존 직무를 대체하면서 재학습과 강점의 재설계를 요구한다. "
    "마을과 도시가 재학습 경로를 제공하고 가정과 국가가 전환기 보호를 담당하는 것은 축소되었던 선택권을 다시 분산하여 회복하는 과정이다. "
    "에릭슨은 중장년기의 과제를 생산성 대 침체(generativity vs. stagnation)로 규정했다. 자신이 축적한 것을 새 방향으로 확장하지 못하면 정체와 고립에 빠진다. "
    "이 과제에 응답하는 것은 개인의 의지가 아니라 재학습 기회를 제공하고 새 역할을 연결하며 전환기의 불안을 분담하는 공동체의 구조다.")

h2("4.6", "노년기")
para(
    "농업 시대의 노년은 봉양의 대상, 산업화 시대의 노년은 은퇴와 부양의 대상이었고 두 시대 모두 노인을 수혜자로 규정했다. "
    "디지털 시대는 건강을 미리 예측·예방하면서 존엄과 판단의 주도권을 유지하고, 경험을 다음 세대에 전수하며 세대를 연결하는 능동적 전수자로서의 노년을 제시한다. "
    "에릭슨은 노년기의 과제를 자아통합 대 절망(ego integrity vs. despair)으로 규정했다. 삶 전체를 통합적으로 수용하지 못하면 절망에 빠진다. "
    "전수 통로와 역할이 주어지고 관계 안에 머물 수 있는 공동체 구조가 통합을 가능하게 한다. "
    "가정은 돌봄과 존엄의 생활 공간을, 이웃은 관계의 지속을, 마을과 도시는 멘토와 자문 역할을 담당하고 국가는 소득의 최소 기반과 고립 방지를 맡는다. "
    "폴라니의 개념으로 보면 노인이 시장과 국가 연금에만 의존하는 상태는 탈배태이며, 재배태는 노인이 관계와 역할의 그물로 회복되는 것이다.")
...
h2("5.1", "가정")
para(
    "가정은 개인이 태어나는 순간부터 가장 먼저 권리를 위임받는 공동체다. 농업 시대에 가정은 노동·양육·부양·교육을 모두 담당했고, "
    "산업화 시대에는 교육을 학교에, 의료를 병원에 위임하면서 역할이 축소되었다. 디지털 시대 가정의 핵심 역할은 정체성의 단서를 가장 먼저 읽는 생활 기반이다. "
    "아이가 무엇에 반응하고 어떤 활동에서 활기를 보이는지를 가까이에서 관찰하고 기록한다. 또한 중장년의 전환기와 노년의 돌봄·존엄을 지지한다. "
    "브론펜브레너가 미시체계(microsystem)로 규정한, 개인이 직접 속하는 가장 가까운 환경이 가정이며, 이 층위가 건강해야 다른 공동체의 위임도 의미를 갖는다.")

h2("5.2", "이웃")
para(
    "이웃은 산업화 시대에 가장 크게 위축된 공동체다. 표준 제도가 모든 것을 조직하면서 이웃 간 상호 돌봄과 협동 기능이 소멸했다. "
    "정체성 발휘의 시대에 이웃은 아이가 흥미를 실험하고 성인이 새 관계를 맺는 일상망으로서 중요성을 회복한다. "
    "관심 기반 모임, 생활 멘토, 상호 돌봄이 이웃 단위에서 이루어질 때 개인은 발견의 기회를 얻으며, 단절된 사람이 관계망으로 복귀하는 첫 경로도 이웃이다. "
    "오스트롬이 분석한 스위스 알프스 목초지 공동 관리와 일본 어촌의 어장 자치는 이웃 규모의 자치가 정교하게 작동할 수 있음을 보여준다.")

h2("5.3", "마을")
para(
    "마을은 생애주기의 여러 단계에서 훈련과 재학습을 담당하는 공동체다. 산업화 시대에 도시에 기능을 넘기고 행정 단위로 축소되었던 마을은 "
    "디지털 시대에 발견된 방향을 훈련으로 연결하는 생활권으로 재편된다. 청소년에게는 마을 멘토단과 훈련 경로가, 중장년에게는 재학습 경로가 마을을 통해 제공된다. "
    "마을은 중앙 정부가 직접 설계하기보다 구성원이 자치적으로 훈련 경로를 만들고 국가가 최소 기준을 보장할 때 가장 잘 작동한다. "
    "센과 누스바움이 강조한 역량은 마을 단위의 훈련 환경이 갖춰질 때 구체적으로 형성된다.")

h2("5.4", "도시")
para(
    "도시는 산업화 시대에 학교와 공장을 집적하여 표준 인재를 생산·배치하는 기관이었다. 디지털 시대에 도시는 정체성을 실제 성과로 발휘하는 장이자 생활 기반으로 전환된다. "
    "프로젝트 연결, 직무 전환, 생활 조정이 도시의 기능이 되며, 청년의 첫 성과와 중장년의 재설계가 도시를 통해 이루어진다. "
    "도시에 위임되는 권리의 성격은 표준 경로에 자신을 맞추는 것에서 자신의 성과를 발휘할 기반을 만드는 것으로 바뀐다.")
para(
    "그러나 모든 기능이 한 대도시에 집중되면 개인이 위임한 권리도 거대한 행정 단위에 묶여 회수하기 어려워진다. "
    "이는 국가로의 권리 집중과 동일한 문제를 도시 규모에서 반복하는 것이다. 개인은 자신이 어디에 무엇을 맡겼는지 확인하고 필요하면 회수할 수 있는 규모 안에서만 권리를 능동적으로 위임한다. "
    "따라서 디지털 시대의 도시는 단일 대도시가 아니라 체감 가능한 규모의 강소도시들이 연합하는 형태로 재편되어야 한다. "
    "강소도시는 한 사람의 강점을 알아보고 성과의 기회를 연결할 만큼 작고 구체적인 생활권이며, 이 규모에서는 위임한 결정권이 어떻게 쓰이는지 확인하고 조정할 수 있다. "
    "동시에 작은 도시 하나만으로는 시장과 데이터, 기회의 폭이 부족하므로 강소도시들은 데이터·인공지능·행정·산업 기반을 연결해 하나의 연합 생활권을 구성한다. "
    "발휘의 폭은 연합 전체에서 확보하되 권리의 위임과 회수는 개인이 속한 작은 도시 단위에서 이루어지는 구조다. "
    "규모의 효율은 연합으로 확보하고 권리의 통제권은 개인 가까이에 남기는 것이 대도시를 강소도시의 연합으로 재편해야 하는 근거다.")

h2("5.5", "국가와 세계")
para(
    "국가는 산업화 시대 권리 집중의 핵심 기구였다. 교육·고용·복지·의료를 중앙에서 일괄 관리했고 개인이 권리를 위임할 공동체를 선택할 여지를 거의 남기지 않았다. "
    "디지털 시대의 국가는 모든 것을 관리하는 역할에서 물러나, 가정·이웃·마을·도시가 각자의 역할을 수행하도록 최소 조건을 보장하고 이들을 조율하는 "
    "메타 거버넌스(meta-governance)로 전환되어야 한다. 모든 사람에게 권리 위임의 기회가 열리도록 최소 권리선을 설정하고, 어느 공동체도 권리를 과도하게 독점하지 않도록 규칙을 만드는 것이 국가의 역할이다. "
    "에스핑안데르센이 제시한 사회투자 국가, 곧 소득 보전에서 역량 형성으로의 전환이 그 구체적 형태다.")
para(
    "세계는 산업화 시대에 국가 간 협정으로만 작동하던 추상적 층위였다. 디지털 시대에 세계는 처음으로 개인의 권리 위임 상대로 등장한다. "
    "국경을 넘는 협업이 가능해지면서 청년과 중장년이 세계 무대에서 성과를 발휘할 수 있다. 동시에 기후변화·인공지능 위험·감염병처럼 국가 단위를 넘는 공동 위험이 커지면서 "
    "세계는 모든 생애단계의 안전과 직결된 거버넌스 층위가 된다. 오스트롬이 만년에 분석한 기후변화 거버넌스가 보여주듯 거버넌스 층위가 분산될수록 공동 위험에 대한 적응력이 높아진다.")
...
rich([("첫째, 위임의 선택권을 개인에게 돌려주어야 한다. ", True),
      ("국가가 모든 생애단계의 필요를 독점 관리하는 구조에서, 개인이 필요에 따라 가정·이웃·마을·도시에 권리를 분산 위임하고 상황이 바뀌면 회수할 수 있는 구조로 전환해야 한다. "
       "로크가 제시했듯 위임은 항상 회수 가능해야 한다. 탈상품화(decommodification), 곧 시장에 기대지 않고도 살아갈 수 있게 하는 사회 보장만으로는 부족하며, 위임 대상을 선택하고 변경할 수 있는 구조가 함께 필요하다.", False)])
...
para(
    "루소의 자치, 로크의 신탁 회수 가능성, 오스트롬의 다중심 자치, 브론펜브레너의 근접 과정, 폴라니의 재배태는 같은 방향을 가리킨다. "
    "개인은 필요에 맞는 공동체를 구성하고 그 공동체에 권리를 위임하며 상황이 바뀌면 권리를 회수할 수 있어야 한다.")
