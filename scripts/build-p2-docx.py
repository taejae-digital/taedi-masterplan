...
para("— 생애 6단계 · 6공동체 · 4계층으로 본 사회 재편의 원리 —",
...
para("공동체가 개인을 빚는 것이 아니라, 개인의 생애주기적 필요가 공동체의 모양을 빚는다. 사람은 시대마다 다른 것을 필요로 했고, "
     "그 필요가 가정에서 세계에 이르는 공동체의 형태와 역할을 바꿔 왔다. 이 글은 그 명제를 생애 6단계, 6공동체, 4계층의 세 축으로 풀어내고, "
...
rich([("키워드  ", True),
      ("생애주기, 공동체 전환, 정체성 실현, 4계층 격차, 디지털 전환, 역량과 자본", False)],
...
para("이 글은 먼저 세 축(생애 6단계·6공동체·4계층)을 세우고, 세 시대에 걸친 공동체 역할의 이동을 비교한 뒤, 각 생애단계에서 기술이 만든 "
     "필요와 공동체의 응답, 그리고 계층 격차를 들여다보고, 디지털 전환이 공동체에 요구하는 재배치의 원리를 도출한다.")

# ════════════════════════ 2. 세 축 ════════════════════════
h1("2.", "분석의 세 축 — 생애 6단계 · 6공동체 · 4계층")

h2("2.1", "생애 6단계 — 필요는 단계마다 달라진다")
...
h2("2.3", "4계층 — 같은 필요, 다른 충족")
para("같은 생애단계라도 사람마다 그 필요를 충족받는 정도는 다르다. 우리는 이 차이를 네 계층으로 포착한다. 리더는 자본과 조직과 기회를 "
     "먼저 쥐고 제도 안에서 우위를 누린다. 프로는 제도를 성실히 따르되 점수와 직무로 평가받는다. 아마는 자원이 부족해 통로가 자주 막힌다. "
     "소외는 제도와 시장에서 아예 배제되어 기여할 길 자체가 닫힌다.", indent_first=None)
para("중요한 것은 소외 계층이 잠재력이 없는 사람이 아니라는 점이다. 누구나 모든 가능성을 지니고 태어난다고 명시한다. "
     "소외는 능력의 부재가 아니라, 그 능력이 실제 기능으로 전환될 조건을 공동체가 제공하지 못한 결과다. "
     "이 구분은 아마르티아 센과 마사 누스바움의 역량접근법으로 정확히 설명된다. 자원을 가졌느냐가 아니라 ‘할 수 있게 되었느냐’가 관건이며, "
     "동일한 자원도 사람이 처한 조건에 따라 다른 기능으로 전환된다.")
para("계층 격차가 생애 전체에 걸쳐 재생산되는 메커니즘은 피에르 부르디외의 자본이론이 밝혀준다. 리더 계층은 경제·문화·사회자본을 두루 갖추고, "
     "소외 계층은 그 셋 모두의 결핍 속에서 가능성의 지평 자체가 좁게 설정된다. 유아기에 ‘발견을 도울 환경이 없던 아이’가 중장년에 이르러 "
     "‘경력 단절이 곧 사회적 퇴출로 이어지는 사람’이 되는 경로는, 격차가 한 시점의 문제가 아니라 생애에 걸친 누적임을 보여준다.")
...
para("디지털 기술은 이 단계에서 개인의 출발선을 바꾼다. 생체·행동 데이터를 읽는 기술이 등장하면서, 아이가 타고난 기질과 강점의 단서를 "
     ...
     "그러나 리더 계층의 아이는 데이터로 단서가 풍부하게 읽히는 반면, 소외 계층의 아이는 데이터도 관찰도 닿지 않는 곳에 남는다. "
...
para("농업 시대의 아이는 집안일과 농사일을 익히며 자랐고, 산업화 시대의 아이는 취학 준비와 또래에 뒤지지 않는 기초 학력에 매였다. "
     ...
     "디지털 시대에는 가정·이웃·마을·도시·국가가 함께 역할을 나눈다. 리더 계층은 폭넓은 체험으로 강점을 또렷이 하고, 아마 계층은 마을 공동 "
     "프로그램으로, 소외 계층은 국가와 마을이 먼저 채우는 체험과 돌봄으로 출발선에 선다. 이 분산 구조는 브론펜브레너가 말한 중간체계—가정·이웃·마을이 "
...
para("이 단계는 정체성의 위기가 가장 첨예한 시기다. 산업화 시대의 청소년은 입시 경쟁에서 좋은 성적과 상급 학교 진학에 모든 것을 걸었다. "
     "리더는 원하는 진로를 골랐지만, 프로는 점수에 진로를 빼앗기고, 아마는 성적에 밀려 다른 재능을 묻고, 소외는 진학 탈락으로 길이 막혔다. "
     "마이클 영이 경고한 능력주의의 함정이 바로 이 장면이다. 시험이 삶을 재단하고, 실패는 구조가 아니라 개인의 능력 부족으로 귀속되어 "
     "소외 계층에게 이중의 고통을 안긴다. 디지털 시대는 연결 공동체를 도시의 입시 기관에서 마을의 멘토와 훈련 경로로 옮긴다. "
...
para("디지털 기술은 이 단계에서 개인이 일하는 방식을 가장 크게 바꾼다. AI·데이터·플랫폼이 기획·제작·유통의 비용을 낮추면서, 개인은 큰 조직에 "
     ...
     "국경을 넘는 협업 시장이 열리면서 국내 노동시장의 협소함을 우회할 길이 생기기 때문이다. 리더는 창업과 전문직으로 도시와 세계 무대에서 발휘하지만, "
     "소외 계층에게는 진입 장벽을 낮춰 첫 기여를 잇는 지원이 필요하다. 이는 에스핑안데르센이 지식경제 복지국가에서 강조한 ‘사회적 투자’—역량이 기여로 "
...
para("이 단계는 디지털 전환의 혜택과 위험이 가장 불균등하게 갈리는 시기다. 산업화 시대의 목표는 고용 유지와 가족 부양, 사회적 지위였다. "
     "디지털 시대의 목표는 강점을 재설계하고 새 영역에 도전하는 것이다. 그러나 프로 계층은 재학습 없이 기술 변화 앞에 버티고, 아마 계층은 고용 불안에 "
     "경력 중단의 위험을 안으며, 소외 계층은 실직이 곧 사회적 퇴출로 이어져 회복이 어렵다. 부르디외의 관점에서 보면, 중장년기의 전환 실패는 "
     "생애에 걸쳐 쌓인 자본 격차가 임계점을 넘어 회복 불가능한 붕괴로 치닫는 국면이다. 이에 대해 우리는 마을의 재학습 접근과 국가·가정의 전환기 "
     "보호로 응답한다.", indent_first=None)

h2("4.6", "노년 — 전수하고 연결하는 시기")
para("농업 시대의 노인은 권위와 봉양을 누렸고, 산업화 시대의 노인은 은퇴와 함께 부양의 대상으로 남았다. 디지털 시대의 노년기 재정의는 과감하다. "
     "건강을 미리 예측하고 예방하며, 존엄과 판단의 주도권을 지키는 시기로 본다. 리더 계층은 멘토와 자문으로 후속 세대와 경험을 잇는 현역 전수자가 된다. "
     "이 단계에는 가정의 예방·돌봄·존엄, 이웃의 관계 유지, 도시의 건강 예측·생활 의료, 국가의 소득·최소 권리선이 역할을 나눈다. "
     "에스핑안데르센의 탈상품화 원리—노인이 시장에 의존하지 않고도 인간다운 삶을 유지해야 한다는—는 소외 노인을 고립에서 건져 돌봄과 역할로 다시 잇는 "
...
para("디지털 시대의 생애주기는 한 방향으로 자라는 직선이 아니라, 앞 단계의 결과가 다음 단계의 조건이 되는 나선형 누적 구조다. "
     ...
     "그래서 초기 단계의 계층 격차는 단순한 기회 불평등이 아니라 생애 전체 사이클의 단절을 뜻한다. 초기에 역량이 기능으로 전환되지 못하면 "
...
rich([("둘째, 소외 계층에게는 표준이 아니라 맞춤이 필요하다. ", True),
      ("기다리는 제도가 아니라 ‘먼저 찾아가는’ 공동체의 능동성이 소외를 줄인다.", False)])
...
rich([("셋째, 소외 계층에게 선제적·능동적으로 다가간다. ", True),
...
h2("6.2", "한계")
para("이 분석에는 한계가 있다. 생애단계와 계층 구분은 이상형이어서 현실의 경계는 훨씬 유동적이며, 특히 청년기와 중장년기의 구분이 점점 흐려지고 있다. "
...
refs = [
 ...
 "태재미래전략연구원. (2026). 마스터플랜 내부 데이터: 개인 생애주기·6공동체·4계층 데이터 [내부 자료].",
]
for r in refs:
    p = doc.add_paragraph()
    pf = p.paragraph_format
