...
para(
    "유리 브론펜브레너(Urie Bronfenbrenner, 1917-2005)는 개인의 발달을 가정(미시체계), 이웃과 학교(중간체계), 부모의 직장(외체계), 사회의 가치 규범(거시체계)이 상호작용하는 중첩된 생태체계로 분석했다(『인간 발달의 생태학』, 1979). 그의 핵심 명제는 개인에게 가장 큰 영향을 미치는 것이 가까운 공동체와의 직접적 상호작용, 곧 근접 과정(proximal process)이라는 것이다. 산업화 시대는 근접 과정이 일어나는 가정·이웃·마을을 약화시키고 권리를 국가로 집중함으로써 개인 발달의 토대를 약화시켰다.")
...
para(
    ...
    "예스타 에스핑안데르센(Gøsta Esping-Andersen, 1947- )은 지식경제에서는 복지를 소득 보전이 아니라 교육·보육·재훈련 같은 사전 투자, 곧 사회적 투자(social investment)로 전환해야 한다고 강조했다.")
...
para(
    ...
    "폴라니의 개념으로 보면 노인이 시장과 국가 연금에만 의존하는 상태는 탈배태이며, 재배태는 노인을 관계와 역할의 그물로 되돌리는 것이다.")
...
para(
    ...
    "디지털 시대의 국가는 모든 것을 관리하는 역할에서 물러나, 가정·이웃·마을·도시가 각자의 역할을 수행하도록 최소 조건을 보장하고 조율하는 메타 거버넌스(meta-governance)로 전환되어야 한다. 모든 사람에게 권리 위임의 기회가 열리도록 최소 권리선을 설정하고 어느 공동체도 권리를 과도하게 독점하지 않도록 규칙을 만드는 것이 국가의 역할이며, 에스핑안데르센이 제시한 사회적 투자 국가, 곧 소득 보전에서 역량 형성으로의 전환이 그 구체적 형태다.")
para(
