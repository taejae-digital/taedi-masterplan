...
para(
    ...
    "한국의 품앗이는 비공식적이지만 정교한 권리 위임과 회수의 사례다. 생애주기도 협동의 논리를 따라 아동기는 공동 노동의 준비기, 청년기는 새 가정 구성기, 중장년은 마을 자치의 중심, 노년은 봉양의 대상이었다.")
...
para(
    ...
    "관심 기반 모임과 상호 돌봄이 이웃 단위에서 이루어질 때 개인은 발견의 기회를 얻으며, 단절된 사람이 관계망으로 복귀하는 첫 경로도 이웃이다. 오스트롬이 분석한 스위스 알프스 목초지와 일본 어촌의 어장 자치는 이웃 규모의 자치가 정교하게 작동함을 보여준다.")
...
para(
    ...
    "작은 도시 하나만으로는 시장과 데이터의 폭이 부족하므로 강소도시들은 데이터·인공지능·행정·산업 기반을 연결해 하나의 연합 생활권을 구성한다. 발휘의 폭은 연합에서 확보하되 권리의 위임과 회수는 개인이 속한 작은 도시 단위에서 이루어지는 구조다. 규모의 효율은 연합으로, 권리의 통제권은 개인 가까이에 남기는 것이 대도시를 강소도시 연합으로 재편해야 하는 근거다.")
